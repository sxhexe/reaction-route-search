...
namespace OpenBabel
{
  ...
  bool GaussianInputFormat::ReadMolecule(OBBase* pOb, OBConversion* pConv)
  {
    ...
    while (!std::getline(ifs, line)) {
      ...
      OBPairData* keywords = new OBPairData;
      keywords->SetAttribute("keywords");
      keywords->SetValue(line);
      keywords->SetOrigin(fileformatInput);
      pmol->SetData(keywords);
    }

    std::getline(ifs, line);
    std::getline(ifs, line);
    OBPairData* title = new OBPairData;
    title->SetAttribute("title");
    title->SetValue(line);
    title->SetOrigin(fileformatInput);
    pmol->SetData(title);
