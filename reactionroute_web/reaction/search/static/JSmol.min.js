(function(a, m) {
    ...
    function l(f) {
        var w = f.length,
            a = g.type(f);
        return "function" === a || g.isWindow(f) ? !1 : 1 === f.nodeType && w ? !0 : "array" === a || 0 === w || "number" === typeof w && 0 < w && w - 1 in f
    }

    function h(f, w, a) {
        if (g.isFunction(w)) return g.grep(f, function(f, g) {
            return !!w.call(f,
                g, f) !== a
        });
        if (w.nodeType) return g.grep(f, function(f) {
            return f === w !== a
        });
        if ("string" === typeof w) {
            if (Vc.test(w)) return g.filter(w, f, a);
            w = g.filter(w, f)
        }
        return g.grep(f, function(f) {
            return 0 <= g.inArray(f, w) !== a
        })
    }

    function e(f, w) {
        do f = f[w]; while (f && 1 !== f.nodeType);
        return f
    }

    function k() {
        y.addEventListener ? (y.removeEventListener("DOMContentLoaded", q, !1), a.removeEventListener("load", q, !1)) : (y.detachEvent("onreadystatechange", q), a.detachEvent("onload", q))
    }

    function q() {
        if (y.addEventListener || "load" === event.type ||
            "complete" === y.readyState) k(), g.ready()
    }

    function s(f, w, a) {
        if (void 0 === a && 1 === f.nodeType)
            if (a = "data-" + w.replace(Wc, "-$1").toLowerCase(), a = f.getAttribute(a), "string" === typeof a) {
                try {
    ...
    function b(f, a, p, A) {
        if (g.acceptData(f)) {
            var b = g.expando,
                ...
                n =
                c ? f[b] : f[b] && b;
            if (n && d[n] && (A || d[n].data) || !(void 0 === p && "string" === typeof a)) {
                n || (n = c ? f[b] = j.pop() || g.guid++ : b);
                d[n] || (d[n] = c ? {} : {
                    toJSON: g.noop
                });
                if ("object" === typeof a || "function" === typeof a) A ? d[n] = g.extend(d[n], a) : d[n].data = g.extend(d[n].data, a);
                f = d[n];
                A || (f.data || (f.data = {}), f = f.data);
    ...
    function c(f, a, p) {
        if (g.acceptData(f)) {
            var A, b, c = f.nodeType,
                j = c ? g.cache : f,
                d = c ? f[g.expando] : g.expando;
            if (j[d]) {
                if (a &&
                    (A = p ? j[d] : j[d].data)) {
                    g.isArray(a) ? a = a.concat(g.map(a, g.camelCase)) : a in A ? a = [a] : (a = g.camelCase(a), a = a in A ? [a] : a.split(" "));
                    for (b = a.length; b--;) delete A[a[b]];
                    if (p ? !u(A) : !g.isEmptyObject(A)) return
                }
                if (!p && (delete j[d].data, !u(j[d]))) return;
                c ? g.cleanData([f], !0) : v.deleteExpando || j != j.window ? delete j[d] : j[d] = null
            }
        }
    }
    ...
    function K(f, a) {
        var p, A, b = 0,
            c = typeof f.getElementsByTagName !== ka ? f.getElementsByTagName(a || "*") : typeof f.querySelectorAll !== ka ? f.querySelectorAll(a || "*") : void 0;
        if (!c) {
            c = [];
            for (p = f.childNodes || f; null != (A = p[b]); b++) !a || g.nodeName(A, a) ? c.push(A) : g.merge(c, K(A, a))
        }
    ...
    function P(f, a) {
        for (var p, A = 0; null != (p = f[A]); A++) g._data(p, "globalEval", !a || g._data(a[A], "globalEval"))
    }
    ...
    function Fa(f, a) {
        if (1 === a.nodeType && g.hasData(f)) {
            ...
            var c = g._data(a, A),
                j = A.events;
            if (j)
                for (p in delete c.handle, c.events = {}, j) {
                    A = 0;
                    for (b = j[p].length; A < b; A++) g.event.add(a,
                        p, j[p][A])
                }
    ...
    function Pa(f, w) {
        var p = g(w.createElement(f)).appendTo(w.body),
            A = a.getDefaultComputedStyle ? a.getDefaultComputedStyle(p[0]).display : g.css(p[0], "display");
        p.detach();
        return A
    }
    ...
    function gb(f, a) {
        if (a in f) return a;
        for (var p = a.charAt(0).toUpperCase() + a.slice(1), A = a, b = oc.length; b--;)
            if (a = oc[b] + p, a in f) return a;
        return A
    }

    function va(f, a) {
        for (var p, A, b, c = [], j = 0, d = f.length; j < d; j++)
            if (A = f[j], A.style)
                if (c[j] = g._data(A, "olddisplay"), p = A.style.display, a) !c[j] && "none" === p && (A.style.display = ""), "" === A.style.display && hb(A) && (c[j] = g._data(A, "olddisplay", db(A.nodeName)));
                else if (!c[j] && (b = hb(A), p && "none" !== p || !b)) g._data(A, "olddisplay", b ? p : g.css(A, "display"));
        for (j = 0; j < d; j++)
            if (A = f[j], A.style && (!a || "none" === A.style.display || "" === A.style.display)) A.style.display = a ? c[j] || "" : "none";
        return f
    }
    ...
    function x(f, a, p, b, c) {
        ...
        for (var j = 0; 4 > a; a += 2) "margin" === p && (j += g.css(f, p + Qa[a], !0, c)), b ? ("content" === p && (j -= g.css(f, "padding" + Qa[a], !0, c)), "margin" !== p && (j -=
            g.css(f, "border" + Qa[a] + "Width", !0, c))) : (j += g.css(f, "padding" + Qa[a], !0, c), "padding" !== p && (j += g.css(f, "border" + Qa[a] + "Width", !0, c)));
        return j
    }
    ...
    function Z(f, a, p) {
        var b = !0,
            ...
            j = Ra(f),
            d = v.boxSizing() && "border-box" === g.css(f, "boxSizing", !1, j);
        if (0 >= c || null == c) {
            c = Sa(f, a, j);
            if (0 > c || null == c) c = f.style[a];
            if (sb.test(c)) return c;
            b = d && (v.boxSizingReliable() || c === f.style[a]);
            c = parseFloat(c) || 0
        }
        return c + x(f, a, p || (d ? "border" : "content"), b, j) + "px"
    }
    ...
    function wa(f, a, p) {
        for (var b, g = (ib[a] || []).concat(ib["*"]), c = 0, j = g.length; c < j; c++)
    ...
    function jb(f, a, p) {
        var b, c, j = 0,
            ...
            e = function() {
                if (c) return !1;
                for (var a = Za || fa(), a = Math.max(0, r.startTime + r.duration - a), w =
                        1 - (a / r.duration || 0), p = 0, b = r.tweens.length; p < b; p++) r.tweens[p].run(w);
                n.notifyWith(f, [r, w, a]);
                if (1 > w && b) return a;
                n.resolveWith(f, [r]);
                return !1
            },
            r = n.promise({
                ...
                createTween: function(a, w) {
                    var p = g.Tween(f, r.opts, a, w, r.opts.specialEasing[a] || r.opts.easing);
                    r.tweens.push(p);
                    return p
                },
                stop: function(a) {
        ...
        var h, t, l, k;
        for (b in a)
            if (h = g.camelCase(b), t = p[h], l = a[b], g.isArray(l) && (t = l[1], l = a[b] = l[0]), b !== h && (a[h] = l, delete a[b]), (k = g.cssHooks[h]) && "expand" in k)
                for (b in l = k.expand(l), delete a[h], l) b in a || (a[b] = l[b], p[b] = t);
            else p[h] = t;
        for (; j < d; j++)
            if (b = tb[j].call(r, f, a, r.opts)) return b;
        g.map(a, wa, r);
    ...
    function kb(f) {
        return function(a, p) {
            ...
            var b, c = 0,
                j = a.toLowerCase().match(xa) || [];
            if (g.isFunction(p))
                for (; b = j[c++];) "+" === b.charAt(0) ? (b = b.slice(1) || "*", (f[b] = f[b] || []).unshift(p)) : (f[b] = f[b] || []).push(p)
        }
    }
    ...
    function ub(f, a, p, b) {
        function c(n) {
            ...
            g.each(f[n] || [], function(f, g) {
                var n = g(a, p, b);
                if ("string" === typeof n && !d && !j[n]) return a.dataTypes.unshift(n),
                    c(n), !1;
                if (d) return !(e = n)
            });
            return e
    ...
    function ma(f, a, p, b) {
        var c;
        if (g.isArray(a)) g.each(a, function(a, w) {
            p || ad.test(f) ? b(f, w) : ma(f + "[" + ("object" === typeof w ? a : "") + "]", w, p, b)
        });
        else if (!p && "object" === g.type(a))
            for (c in a) ma(f + "[" + c + "]", a[c], p, b);
        else b(f, a)
    }
    ...
    g.extend = g.fn.extend = function() {
        var f, a, p, b, c, j = arguments[0] || {},
            d = 1,
            n = arguments.length,
            e = !1;
        "boolean" === typeof j && (e = j, j = arguments[d] || {}, d++);
        "object" !== typeof j && !g.isFunction(j) && (j = {});
        d === n && (j = this, d--);
        for (; d < n; d++)
            if (null != (c = arguments[d]))
                for (b in c) f = j[b], p = c[b], j !== p && (e && p && (g.isPlainObject(p) || (a = g.isArray(p))) ? (a ?
                    (a = !1, f = f && g.isArray(f) ? f : []) : f = f && g.isPlainObject(f) ? f : {}, j[b] = g.extend(e, f, p)) : void 0 !== p && (j[b] = p));
        return j
    };
    g.extend({
        ...
        type: function(f) {
            return null == f ? f + "" : "object" === typeof f || "function" === typeof f ? Q[bd.call(f)] || "object" : typeof f
        },
        ...
        grep: function(f, a, p) {
            for (var b = [], g = 0, c =
                    f.length, j = !p; g < c; g++) p = !a(f[g], g), p !== j && b.push(f[g]);
            return b
        },
        map: function(f, a, p) {
            var b, g = 0,
                ...
                j = [];
            if (l(f))
                for (; g < c; g++) b = a(f[g], g, p), null != b && j.push(b);
            else
                for (g in f) b = a(f[g], g, p), null != b && j.push(b);
            return r.apply([], j)
        },
        guid: 1,
        proxy: function(f, a) {
    ...
    var Pb = a,
        E = function(f, a, p, b) {
            ...
            if (X && !b) {
                if (g = gd.exec(f))
                    if (j = g[1])
                        if (9 === d)
                            if ((c = a.getElementById(j)) && c.parentNode) {
                                if (c.id === j) return p.push(c), p
                            } else return p;
                else {
                    if (a.ownerDocument && (c = a.ownerDocument.getElementById(j)) && ra(a,
                            c) && c.id === j) return p.push(c), p
                } else {
                    if (g[2]) return Ga.apply(p, a.getElementsByTagName(f)), p;
                    if ((j = g[3]) && I.getElementsByClassName && a.getElementsByClassName) return Ga.apply(p, a.getElementsByClassName(j)), p
                }
                if (I.qsa && (!S || !S.test(f))) {
                    c = g = M;
                    j = a;
                    ...
                    if (1 === d && "object" !== a.nodeName.toLowerCase()) {
                        ...
                        c = "[id='" + c + "'] ";
                        for (j = d.length; j--;) d[j] = c + vb(d[j]);
                        j = Qb.test(f) && ga(a.parentNode) || a;
                        n = d.join(",")
                    }
                    if (n) try {
                        return Ga.apply(p,
                            j.querySelectorAll(n)), p
                    } catch (e) {} finally {
            ...
            var r;
            a: {
                f = f.replace(wb, "$1");c = sa(f);
                if (!b && 1 === c.length) {
                    g = c[0] = c[0].slice(0);
                    if (2 < g.length && "ID" === (r = g[0]).type && I.getById && 9 === a.nodeType && X && H.relative[g[1].type]) {
                        a = (H.find.ID(r.matches[0].replace(Ha, Ia), a) || [])[0];
                        if (!a) {
                            r = p;
                            break a
                        }
                    ...
                    for (d = xb.needsContext.test(f) ? 0 : g.length; d--;) {
                        r = g[d];
                        if (H.relative[j = r.type]) break;
                        if (j = H.find[j])
                            if (b = j(r.matches[0].replace(Ha, Ia), Qb.test(g[0].type) &&
                                    ga(a.parentNode) || a)) {
                                g.splice(d, 1);
                                f = b.length && vb(g);
                                if (!f) {
                ...
                $a(f, c)(b, a, !X, p, Qb.test(f) && ga(a.parentNode) || a);r = p
            }
            return r
        },
        Rb = function() {
            function f(p, b) {
                a.push(p + " ") > H.cacheLength && delete f[a.shift()];
                return f[p + " "] = b
            }
            var a = [];
            return f
        ...
        Sb = function(f, a) {
            for (var p = f.split("|"), b = f.length; b--;) H.attrHandle[p[b]] =
                a
        },
        pc = function(f, a) {
            var p = a && f,
                b = p && 1 === f.nodeType && 1 === a.nodeType && (~a.sourceIndex || ha) - (~f.sourceIndex || ha);
            if (b) return b;
            if (p)
                for (; p = p.nextSibling;)
                    if (p === a) return -1;
        ...
        ta = function(f) {
            return oa(function(a) {
                a = +a;
                return oa(function(p, b) {
                    for (var g, c = f([], p.length, a), j = c.length; j--;)
                        if (p[g =
                                c[j]]) p[g] = !(b[g] = p[g])
                })
            })
        ...
        sa = function(f, a) {
            var p, b, g, c, j, d, n;
            if (j = Ja[f + " "]) return a ? 0 : j.slice(0);
            j = f;
            d = [];
            for (n = H.preFilter; j;) {
                if (!p || (b = kd.exec(j))) b && (j = j.slice(b[0].length) || j), d.push(g = []);
                p = !1;
                if (b = ld.exec(j)) p = b.shift(), g.push({
                    value: p,
                    type: b[0].replace(wb, " ")
                }), j = j.slice(p.length);
                for (c in H.filter)
                    if ((b = xb[c].exec(j)) && (!n[c] || (b = n[c](b)))) p = b.shift(), g.push({
                        value: p,
                        type: c,
                        matches: b
                    }), j = j.slice(p.length);
            ...
            return a ? j.length : j ? E.error(f) : Ja(f, d).slice(0)
        },
        vb = function(f) {
        ...
        Tb = function(f, a, p) {
            ...
            return a.first ? function(a, p, w) {
                for (; a = a[b];)
                    if (1 === a.nodeType || g) return f(a, p, w)
            } : function(a, p, w) {
                var j, d, n = [ba, c];
                if (w)
                    for (; a = a[b];) {
                        if ((1 === a.nodeType || g) && f(a, p, w)) return !0
                    } else
                        for (; a = a[b];)
                            if (1 === a.nodeType || g) {
                                d = a[M] || (a[M] = {});
                                if ((j = d[b]) && j[0] === ba && j[1] === c) return n[2] = j[2];
                                d[b] = n;
                                if (n[2] = f(a, p,
                                        w)) return !0
                            }
        ...
        Ta = function(f, a, p, b, g) {
            for (var c, j = [], d = 0, n = f.length, e = null != a; d < n; d++)
                if (c = f[d])
                    if (!p || p(c, b, g)) j.push(c), e && a.push(d);
            return j
        },
        Vb = function(f, a, p, b, g, c) {
            b && !b[M] && (b = Vb(b));
            g && !g[M] && (g = Vb(g, c));
            return oa(function(c, j, d, n) {
                var e, r, ea = [],
                    l = [],
                    h = j.length,
                    t;
                if (!(t = c)) {
                    t = a || "*";
                    for (var k = d.nodeType ? [d] : d, B = [], z = 0, T = k.length; z < T; z++) E(t, k[z], B);
                    t = B
                }
                t = f && (c || !a) ? Ta(t, ea, f, d,
                    n) : t;
                ...
                if (c) {
                    if (g || f) {
                        if (g) {
                            e = [];
                            for (d = k.length; d--;)
                                if (r = k[d]) e.push(t[d] = r);
                            g(null, k = [], e, n)
                        }
                        for (d = k.length; d--;)
                            if ((r = k[d]) && -1 < (e = g ? Ua.call(c, r) : ea[d])) c[e] = !(j[e] = r)
                    }
                } else k = Ta(k === j ? k.splice(h, k.length) : k), g ? g(null, j, k, n) : Ga.apply(j, k)
            })
        },
        Wb = function(f) {
            var a, p, b, g = f.length,
                c = H.relative[f[0].type];
            p = c || H.relative[" "];
            for (var j = c ? 1 : 0, d = Tb(function(f) {
                    return f === a
                }, p, !0), n = Tb(function(f) {
                    return -1 <
                        Ua.call(a, f)
                ...
                if (p = H.relative[f[j].type]) e = [Tb(Ub(e), p)];
                else {
                    p = H.filter[f[j].type].apply(null, f[j].matches);
                    if (p[M]) {
                        for (b = ++j; b < g && !H.relative[f[b].type]; b++);
                        return Vb(1 < j && Ub(e), 1 < j && vb(f.slice(0, j - 1).concat({
                            value: " " === f[j - 2].type ? "*" : ""
                        })).replace(wb, "$1"), p, j < b && Wb(f.slice(j, b)), b < g && Wb(f = f.slice(b)), b < g && vb(f))
                    }
                    e.push(p)
                }
            return Ub(e)
        },
        Ba, I, H, Ka, rc, $a, Aa, qa, Ca, na, R, ia, X, S, ca, La, ra, M = "sizzle" + -new Date,
        W = Pb.document,
        ba = 0,
        md = 0,
        sc = Rb(),
    ...
    na = E.setDocument = function(f) {
        ...
        H.find.TAG = I.getElementsByTagName ? function(f, a) {
            ...
            if ("*" === f) {
                for (; b = c[g++];) 1 === b.nodeType && w.push(b);
                return w
            }
            return c
        };
        H.find.CLASS = I.getElementsByClassName && function(f, a) {
            if (typeof a.getElementsByClassName !== T && X) return a.getElementsByClassName(f)
        };
        ca = [];
        S = [];
        if (I.qsa = lb.test(a.querySelectorAll)) pa(function(f) {
        ...
        (I.matchesSelector = lb.test(La = ia.webkitMatchesSelector || ia.mozMatchesSelector || ia.oMatchesSelector || ia.msMatchesSelector)) &&
        pa(function(f) {
            I.disconnectedMatch = La.call(f, "div");
            La.call(f, "[s!='']:x");
            ca.push("!=", Xb)
        });
        S = S.length && RegExp(S.join("|"));
        ca = ca.length && RegExp(ca.join("|"));
        ra = (f = lb.test(ia.compareDocumentPosition)) || lb.test(ia.contains) ? function(f, a) {
            var b = 9 === f.nodeType ? f.documentElement : f,
                w = a && a.parentNode;
            return f === w || !(!w || !(1 === w.nodeType && (b.contains ? b.contains(w) : f.compareDocumentPosition && f.compareDocumentPosition(w) & 16)))
        } : function(f, a) {
            if (a)
                for (; a = a.parentNode;)
                    if (a === f) return !0;
        ...
        B = f ? function(f,
            ...
            return g & 1 || !I.sortDetached && b.compareDocumentPosition(f) === g ? f === a || f.ownerDocument === W && ra(W, f) ? -1 : b === a || b.ownerDocument === W && ra(W, b) ? 1 : qa ? Ua.call(qa, f) - Ua.call(qa, b) : 0 : g & 4 ? -1 : 1
        } : function(f, b) {
            if (f === b) return Ca = !0, 0;
            var g, c = 0;
            g = f.parentNode;
            var j = b.parentNode,
                d = [f],
                n = [b];
            if (!g || !j) return f === a ? -1 : b === a ? 1 : g ? -1 : j ? 1 : qa ? Ua.call(qa,
                f) - Ua.call(qa, b) : 0;
            if (g === j) return pc(f, b);
    ...
    E.matches = function(f, a) {
        return E(f, null, null, a)
    };
    E.matchesSelector = function(f, a) {
        (f.ownerDocument || f) !== R && na(f);
        a = a.replace(od, "='$1']");
        if (I.matchesSelector && X && (!ca || !ca.test(a)) && (!S || !S.test(a))) try {
            var b = La.call(f, a);
            if (b || I.disconnectedMatch || f.document && 11 !== f.document.nodeType) return b
        } catch (g) {}
        return 0 <
            E(a, R, null, [f]).length
    };
    ...
    E.attr = function(f, a) {
        (f.ownerDocument || f) !== R && na(f);
        var b = H.attrHandle[a.toLowerCase()],
            b = b && da.call(H.attrHandle, a.toLowerCase()) ? b(f, a, !X) : void 0;
        return void 0 !== b ? b : I.attributes || !X ? f.getAttribute(a) : (b = f.getAttributeNode(a)) && b.specified ? b.value : null
    };
    E.error = function(f) {
    ...
    E.uniqueSort = function(f) {
        var a, b = [],
            g = 0,
            c = 0;
        Ca = !I.detectDuplicates;
        qa = !I.sortStable && f.slice(0);
        f.sort(B);
        if (Ca) {
    ...
    H = E.selectors = {
        ...
        preFilter: {
            ...
            CHILD: function(f) {
                f[1] = f[1].toLowerCase();
                "nth" === f[1].slice(0, 3) ? (f[3] || E.error(f[0]), f[4] = +(f[4] ? f[5] + (f[6] || 1) : 2 * ("even" === f[3] || "odd" === f[3])), f[5] = +(f[7] + f[8] || "odd" === f[3])) : f[3] && E.error(f[0]);
                return f
            },
            PSEUDO: function(f) {
                var a, b = !f[5] && f[2];
                ...
                else if (b && pd.test(b) && (a = sa(b, !0)) && (a = b.indexOf(")", b.length - a) - b.length)) f[0] = f[0].slice(0, a), f[2] = b.slice(0, a);
                return f.slice(0, 3)
            }
        },
        filter: {
            ...
            CHILD: function(f, a, b, g, c) {
                ...
                return 1 === g && 0 === c ? function(f) {
                    ...
                    function(a, b, p) {
                        var w, e, r, k, t;
                        b = j !== d ? "nextSibling" : "previousSibling";
                        var l = a.parentNode,
                            h = n && a.nodeName.toLowerCase();
                        p = !p && !n;
                        if (l) {
                            if (j) {
                                for (; b;) {
                                    for (e = a; e = e[b];)
                                        if (n ? e.nodeName.toLowerCase() === h : 1 === e.nodeType) return !1;
                                    t = b = "only" === f && !t && "nextSibling"
                                }
                                return !0
                            }
                            t = [d ? l.firstChild : l.lastChild];
                            if (d && p) {
                                p = l[M] || (l[M] = {});
                                w = p[f] || [];
                                k = w[0] === ba && w[1];
                                r = w[0] === ba && w[2];
                                for (e = k && l.childNodes[k]; e = ++k && e && e[b] || (r = k = 0) || t.pop();)
                                ...
                                for (; e = ++k && e && e[b] || (r = k = 0) || t.pop();)
                                    if ((n ? e.nodeName.toLowerCase() === h : 1 === e.nodeType) && ++r)
                                        if (p && ((e[M] || (e[M] = {}))[f] = [ba, r]), e === a) break;
                            r -= c;
            ...
            PSEUDO: function(f, a) {
                var b, g = H.pseudos[f] || H.setFilters[f.toLowerCase()] || E.error("unsupported pseudo: " + f);
                return g[M] ? g(a) : 1 < g.length ? (b = [f, f, "", a], H.setFilters.hasOwnProperty(f.toLowerCase()) ? oa(function(f, b) {
                        for (var p, c = g(f, a), j = c.length; j--;) p = Ua.call(f, c[j]), f[p] = !(b[p] = c[j])
                    }) :
                    function(f) {
                        return g(f, 0, b)
                    }) : g
        ...
        pseudos: {
            not: oa(function(f) {
                var a = [],
                    b = [],
                    g = $a(f.replace(wb, "$1"));
                return g[M] ? oa(function(f, a, b, p) {
                    p = g(f, null, p, []);
            ...
            has: oa(function(f) {
                return function(a) {
                    return 0 < E(f, a).length
                }
            }),
            contains: oa(function(f) {
                return function(a) {
                    return -1 < (a.textContent || a.innerText || Ka(a)).indexOf(f)
                }
            }),
            lang: oa(function(f) {
                qd.test(f || "") || E.error("unsupported lang: " + f);
                f = f.replace(Ha,
                    Ia).toLowerCase();
                return function(a) {
                    var b;
            ...
            last: ta(function(f, a) {
                return [a - 1]
            }),
            eq: ta(function(f, a, b) {
                return [0 > b ? b + a : b]
            }),
            even: ta(function(f, a) {
    ...
    qc.prototype = H.filters = H.pseudos;
    H.setFilters = new qc;
    $a = E.compile = function(f, a) {
        var b, g = [],
            c = [],
            j = Da[f + " "];
        if (!j) {
            a || (a = sa(f));
            for (b = a.length; b--;) j = Wb(a[b]), j[M] ? g.push(j) : c.push(j);
            var d = 0 < g.length,
                n = 0 < c.length;
            b = function(f, a, b, p, w) {
                var j, e, r, k = 0,
                    ea = "0",
                    t = f && [],
                    l = [],
                    ...
                    B = f || n && H.find.TAG("*",
                        w),
                    z = ba += null == h ? 1 : Math.random() || 0.1,
                    T = B.length;
                for (w && (Aa = a !== R && a); ea !== T && null != (j = B[ea]); ea++) {
                    if (n && j) {
                        for (e = 0; r = c[e++];)
                            if (r(j, a, b)) {
                                p.push(j);
                    ...
                    d && ((j = !r && j) && k--, f && t.push(j))
                }
                k += ea;
                if (d && ea !== k) {
                    for (e = 0; r = g[e++];) r(t, l, a, b);
                    if (f) {
                        if (0 < k)
                            for (; ea--;) !t[ea] && !l[ea] && (l[ea] = ua.call(p));
                        l = Ta(l)
                    }
                    Ga.apply(p, l);
                    w && (!f && 0 < l.length && 1 < k + g.length) && E.uniqueSort(p)
                }
                w && (ba = z, Aa = h);
                return t
            };
    ...
    I.sortStable = M.split("").sort(B).join("") === M;
    I.detectDuplicates = !!Ca;
    na();
    I.sortDetached = pa(function(f) {
    ...
    g.unique = E.uniqueSort;
    g.text = E.getText;
    g.isXMLDoc = E.isXML;
    g.contains = E.contains;
    ...
    g.filter = function(f, a, b) {
        var j = a[0];
        b && (f = ":not(" + f + ")");
        return 1 === a.length && 1 === j.nodeType ? g.find.matchesSelector(j, f) ? [j] : [] : g.find.matches(f, g.grep(a, function(f) {
            return 1 === f.nodeType
        }))
    };
    g.fn.extend({
        find: function(f) {
            ...
            if ("string" !== typeof f) return this.pushStack(g(f).filter(function() {
                for (a = 0; a < c; a++)
                    if (g.contains(j[a], this)) return !0
            }));
            for (a = 0; a < c; a++) g.find(f, j[a], b);
            b = this.pushStack(1 < c ? g.unique(b) : b);
        ...
        if ("string" === typeof f) {
            if ((b = "<" === f.charAt(0) && ">" === f.charAt(f.length - 1) && 3 <= f.length ? [null, f, null] : td.exec(f)) && (b[1] || !a)) {
                if (b[1]) {
                    ...
                    if ((j = y.getElementById(b[2])) && j.parentNode) {
                        if (j.id !== b[2]) return mb.find(f);
                        this.length = 1;
                        this[0] = j
                    }
                    this.context = y;
    ...
    g.extend({
        dir: function(f, a, b) {
            var j = [];
            for (f = f[a]; f && 9 !== f.nodeType && (void 0 === b || 1 !== f.nodeType || !g(f).is(b));) 1 === f.nodeType && j.push(f), f = f[a];
            return j
        },
    ...
    g.fn.extend({
        ...
        closest: function(f, a) {
            for (var b, j = 0, c = this.length, d = [], n = wc.test(f) || "string" !== typeof f ? g(f, a || this.context) : 0; j < c; j++)
                for (b = this[j]; b && b !== a; b = b.parentNode)
    ...
    g.each({
        ...
        g.fn[f] = function(b,
            j) {
            var c = g.map(this, a, b);
            "Until" !== f.slice(-5) && (j = b);
            j && "string" === typeof j && (c = g.filter(j, c));
            1 < this.length && (vd[f] || (c = g.unique(c)), ud.test(f) && (c = c.reverse()));
    ...
    g.Callbacks = function(f) {
        ...
        var j, c, d, n, e, r, k = [],
            ...
            t = function(a) {
                ...
                j = !1;
                k && (l ? l.length && t(l.shift()) : c ? k = [] : h.disable())
            },
            h = {
                add: function() {
                    if (k) {
                        ...
                        (function $c(a) {
                            g.each(a, function(a, b) {
                                var p = g.type(b);
                                "function" === p ? (!f.unique || !h.has(b)) && k.push(b) : b && (b.length && "string" !== p) && $c(b)
                            })
                        })(arguments);
                        j ? n = k.length : c && (r = a, t(c))
                    }
                    return this
                },
                remove: function() {
                    k && g.each(arguments, function(f, a) {
                        for (var b; - 1 < (b = g.inArray(a, k, b));) k.splice(b, 1), j && (b <= n && n--, b <= e && e--)
                    });
                    return this
                },
                has: function(f) {
                ...
                fireWith: function(f, a) {
                    if (k && (!d || l)) a = a || [], a = [f, a.slice ? a.slice() : a], j ? l.push(a) : t(a);
                    return this
                },
                fire: function() {
    ...
    g.extend({
        Deferred: function(f) {
            ...
            g.each(a, function(f, g) {
                ...
                c[g[0]] = function() {
                    c[g[0] + "With"](this === c ? j : this, arguments);
                    return this
                };
                c[g[0] + "With"] = d.fireWith
            });
            j.promise(c);
            f && f.call(c, c);
            return c
        ...
        when: function(f) {
            var a = 0,
                b = n.call(arguments),
                j = b.length,
                c = 1 !== j || f && g.isFunction(f.promise) ? j : 0,
                d = 1 === c ? f : g.Deferred(),
                e = function(f,
                    a, b) {
                    return function(g) {
                        ...
                        b === r ? d.notifyWith(a, b) : --c || d.resolveWith(a, b)
                    }
                },
                r, k, l;
            if (1 < j) {
                r = Array(j);
                k = Array(j);
                for (l = Array(j); a < j; a++) b[a] && g.isFunction(b[a].promise) ? b[a].promise().done(e(a, l, b)).fail(d.reject).progress(e(a, k, r)) : --c
            }
            c || d.resolveWith(l, b);
            return d.promise()
        }
    ...
    g.ready.promise = function(f) {
        if (!yb)
            ...
            else if (y.addEventListener) y.addEventListener("DOMContentLoaded", q, !1), a.addEventListener("load", q, !1);
        else {
            y.attachEvent("onreadystatechange", q);
            a.attachEvent("onload", q);
            var b = !1;
            try {
    ...
    g(function() {
        ...
        if (b) {
            ...
            if (typeof a.style.zoom !== ka && (a.style.cssText =
                    "border:0;margin:0;width:1px;padding:1px;display:inline;zoom:1", v.inlineBlockNeedsLayout = 3 === a.offsetWidth)) b.style.zoom = 1;
            b.removeChild(f)
        }
    });
    ...
    g.fn.extend({
        data: function(f, a) {
            var b, j, c, d = this[0],
                n = d && d.attributes;
            if (void 0 === f) {
                if (this.length && (c = g.data(d), 1 === d.nodeType && !g._data(d, "parsedAttrs"))) {
                    for (b = n.length; b--;) j = n[b].name,
                        0 === j.indexOf("data-") && (j = g.camelCase(j.slice(5)), s(d, j, c[j]));
    ...
    g.extend({
        queue: function(f, a, b) {
            var j;
            if (f) return a = (a || "fx") + "queue", j = g._data(f, a), b && (!j || g.isArray(b) ? j = g._data(f, a, g.makeArray(b)) : j.push(b)), j || []
        },
        dequeue: function(f,
            a) {
            a = a || "fx";
            ...
            "inprogress" === c && (c = b.shift(), j--);
            c && ("fx" === a && b.unshift("inprogress"), delete d.stop, c.call(f, n, d));
            !j && d && d.empty.fire()
        },
    ...
    g.fn.extend({
        ...
        promise: function(f, a) {
            ...
            for (f = f || "fx"; n--;)
                if ((b = g._data(d[n], f + "queueHooks")) && b.empty) j++, b.empty.add(e);
            e();
            return c.promise(a)
        }
    });
    var zb = /[+-]?(?:\d*\.|)\d+(?:[eE][+-]?\d+|)/.source,
        ...
        Va = g.access = function(a, b, j, c, d, n, e) {
            var r = 0,
                k = a.length,
                l = null == j;
            if ("object" === g.type(j))
                for (r in d = !0, j) g.access(a, b, r, j[r], !0, n, e);
            else if (void 0 !== c && (d = !0, g.isFunction(c) || (e = !0), l && (e ? (b.call(a, c), b = null) : (l = b, b = function(a, f, b) {
                    return l.call(g(a), b)
                })), b))
                for (; r < k; r++) b(a[r], j, e ? c : c.call(a[r],
                    r, b(a[r], j)));
            return d ? a : l ? b.call(a) : k ? b(a[0], j) : n
        },
        Mb = /^(?:checkbox|radio)$/i,
    ...
    Y.innerHTML = "  <link/><table></table><a href='/a'>a</a>";
    v.leadingWhitespace = 3 === Y.firstChild.nodeType;
    v.tbody = !Y.getElementsByTagName("tbody").length;
    v.htmlSerialize = !!Y.getElementsByTagName("link").length;
    ...
    v.noCloneChecked = !!Y.cloneNode(!0).lastChild.defaultValue;
    Yb.appendChild(Y);
    Y.innerHTML = "<input type='radio' checked='checked' name='t'/>";
    v.checkClone = Y.cloneNode(!0).cloneNode(!0).lastChild.checked;
    v.noCloneEvent = !0;
    ...
    for (Ab in {
        ...
        if (Bb = "on" + Ab, !(v[Ab + "Bubbles"] = Bb in a)) Ac.setAttribute(Bb, "t"), v[Ab + "Bubbles"] = !1 === Ac.attributes[Bb].expando;
    ...
    g.event = {
        global: {},
        add: function(a, b, j, c, d) {
            var n, e, r, k, l, t, h, B, z;
            if (r = g._data(a)) {
                j.handler && (k = j, j = k.handler, d = k.selector);
                j.guid || (j.guid = g.guid++);
                if (!(e = r.events)) e =
                    r.events = {};
                if (!(l = r.handle)) l = r.handle = function(a) {
                    return typeof g !== ka && (!a || g.event.triggered !== a.type) ? g.event.dispatch.apply(l.elem, arguments) : void 0
                }, l.elem = a;
                b = (b || "").match(xa) || [""];
                for (r = b.length; r--;)
                    if (n = Cc.exec(b[r]) || [], B = t = n[1], z = (n[2] || "").split(".").sort(), B) {
                        n = g.event.special[B] || {};
                        B = (d ? n.delegateType : n.bindType) || B;
                        n = g.event.special[B] || {};
                        t = g.extend({
                                type: B,
                                origType: t,
                            ...
                            k);
                        if (!(h = e[B]))
                            if (h = e[B] = [], h.delegateCount = 0, !n.setup || !1 === n.setup.call(a, c, z, l)) a.addEventListener ? a.addEventListener(B, l, !1) : a.attachEvent && a.attachEvent("on" + B, l);
                        n.add && (n.add.call(a, t), t.handler.guid || (t.handler.guid = j.guid));
                        d ? h.splice(h.delegateCount++, 0, t) : h.push(t);
                        g.event.global[B] = !0
                    }
                a = null
            }
        },
        remove: function(a, b, j, c, d) {
            var n, e, r, k, l, t, h, B, z, T, q, m = g.hasData(a) && g._data(a);
            if (m && (t = m.events)) {
                b = (b || "").match(xa) || [""];
                for (l = b.length; l--;)
                    if (r = Cc.exec(b[l]) || [], z = q = r[1], T = (r[2] || "").split(".").sort(),
                        z) {
                        h = g.event.special[z] || {};
                        z = (c ? h.delegateType : h.bindType) || z;
                        B = t[z] || [];
                        r = r[2] && RegExp("(^|\\.)" + T.join("\\.(?:.*\\.|)") + "(\\.|$)");
                        for (k = n = B.length; n--;)
                            if (e = B[n], (d || q === e.origType) && (!j || j.guid === e.guid) && (!r || r.test(e.namespace)) && (!c || c === e.selector || "**" === c && e.selector)) B.splice(n, 1), e.selector && B.delegateCount--, h.remove && h.remove.call(a, e);
                        k && !B.length && ((!h.teardown || !1 === h.teardown.call(a, T, m.handle)) && g.removeEvent(a, z, m.handle), delete t[z])
                    } else
                        for (z in t) g.event.remove(a, z + b[l], j,
                            c, !0);
                g.isEmptyObject(t) && (delete m.handle, g._removeData(a, "events"))
            }
        },
        trigger: function(f, b, j, c) {
            var d, n, e, r, k, l, t = [j || y],
                h = za.call(f, "type") ? f.type : f;
            k = za.call(f, "namespace") ? f.namespace.split(".") : [];
            e = d = j = j || y;
            if (!(3 === j.nodeType || 8 === j.nodeType) && !Bc.test(h + g.event.triggered))
                if (0 <= h.indexOf(".") && (k = h.split("."), h = k.shift(), k.sort()), n = 0 > h.indexOf(":") && "on" + h, f = f[g.expando] ? f : new g.Event(h, "object" === typeof f && f), f.isTrigger = c ? 2 : 3, f.namespace = k.join("."), f.namespace_re = f.namespace ? RegExp("(^|\\.)" +
                        k.join("\\.(?:.*\\.|)") + "(\\.|$)") : null, f.result = void 0, f.target || (f.target = j), b = null == b ? [f] : g.makeArray(b, [f]), k = g.event.special[h] || {}, c || !(k.trigger && !1 === k.trigger.apply(j, b))) {
                    if (!c && !k.noBubble && !g.isWindow(j)) {
                        r = k.delegateType || h;
                        Bc.test(r + h) || (e = e.parentNode);
                        for (; e; e = e.parentNode) t.push(e), d = e;
                        if (d === (j.ownerDocument || y)) t.push(d.defaultView || d.parentWindow || a)
                    }
                    for (l = 0;
                        (e = t[l++]) && !f.isPropagationStopped();)
                        if (f.type = 1 < l ? r : k.bindType || h, (d = (g._data(e, "events") || {})[f.type] && g._data(e, "handle")) &&
                            d.apply(e, b), (d = n && e[n]) && d.apply && g.acceptData(e)) f.result = d.apply(e, b), !1 === f.result && f.preventDefault();
                    f.type = h;
                    if (!c && !f.isDefaultPrevented() && (!k._default || !1 === k._default.apply(t.pop(), b)) && g.acceptData(j) && n && j[h] && !g.isWindow(j)) {
                        (d = j[n]) && (j[n] = null);
                        g.event.triggered = h;
                        try {
                            j[h]()
        ...
        dispatch: function(a) {
            ...
            if (!(k.preDispatch && !1 === k.preDispatch.call(this, a))) {
                ...
                for (b = 0;
                    (c = e[b++]) && !a.isPropagationStopped();) {
                    a.currentTarget = c.elem;
                    for (d = 0;
                        (j = c.handlers[d++]) && !a.isImmediatePropagationStopped();)
                        if (!a.namespace_re || a.namespace_re.test(j.namespace))
                            if (a.handleObj = j, a.data = j.data, j = ((g.event.special[j.origType] || {}).handle || j.handler).apply(c.elem, r), void 0 !== j && !1 === (a.result = j)) a.preventDefault(), a.stopPropagation()
                }
        ...
        handlers: function(a, b) {
            var j, c, d, n, e = [],
                r = b.delegateCount,
                k = a.target;
            if (r && k.nodeType && (!a.button || "click" !== a.type))
                for (; k != this; k = k.parentNode || this)
                    if (1 === k.nodeType && (!0 !== k.disabled || "click" !== a.type)) {
                        d = [];
                        for (n = 0; n < r; n++) c = b[n], j = c.selector + " ", void 0 === d[j] && (d[j] = c.needsContext ? 0 <= g(j, this).index(k) : g.find(j, this, null, [k]).length), d[j] && d.push(c);
                        d.length && e.push({
                            elem: k,
                            handlers: d
                        })
        ...
        fix: function(a) {
            ...
            c = n.props ? this.props.concat(n.props) : this.props;
            a = new g.Event(d);
            for (b = c.length; b--;) j = c[b], a[j] = d[j];
        ...
        mouseHooks: {
            ...
            filter: function(a, b) {
                var g, j, c = b.button,
                    d = b.fromElement;
                null == a.pageX && null != b.clientX && (g = a.target.ownerDocument || y, j = g.documentElement, g = g.body, a.pageX = b.clientX + (j && j.scrollLeft || g && g.scrollLeft || 0) - (j && j.clientLeft ||
                    g && g.clientLeft || 0), a.pageY = b.clientY + (j && j.scrollTop || g && g.scrollTop || 0) - (j && j.clientTop || g && g.clientTop || 0));
        ...
        simulate: function(a, b, j, c) {
            a = g.extend(new g.Event, j, {
                type: a,
                isSimulated: !0,
                originalEvent: {}
            });
            c ? g.event.trigger(a, null, b) : g.event.dispatch.call(b, a);
            a.isDefaultPrevented() && j.preventDefault()
        }
    };
    g.removeEvent = y.removeEventListener ? function(a, b, g) {
        ...
        b = "on" + b;
        a.detachEvent && (typeof a[b] === ka && (a[b] = null), a.detachEvent(b, g))
    };
    g.Event = function(a, b) {
        if (!(this instanceof g.Event)) return new g.Event(a, b);
        a && a.type ? (this.originalEvent = a, this.type = a.type, this.isDefaultPrevented = a.defaultPrevented || void 0 === a.defaultPrevented && (!1 === a.returnValue || a.getPreventDefault && a.getPreventDefault()) ? d : t) : this.type = a;
        b && g.extend(this, b);
    ...
    g.each({
        ...
        g.event.special[a] = {
            ...
            bindType: b,
            handle: function(a) {
                var f, j = a.relatedTarget,
                    c = a.handleObj;
                if (!j || j !== this && !g.contains(this, j)) a.type = c.origType, f = c.handler.apply(this, arguments), a.type = b;
                return f
            }
        }
    });
    v.submitBubbles || (g.event.special.submit = {
        setup: function() {
        ...
        g.event.special[b] = {
            ...
            teardown: function() {
                var c =
                    ...
                    d = g._data(c, b) - 1;
                d ? g._data(c, b, d) : (c.removeEventListener(a, j, !0), g._removeData(c, b))
            }
        }
    });
    g.fn.extend({
        on: function(a, b, j, c, d) {
            var n, e;
            if ("object" === typeof a) {
                "string" !== typeof b && (j = j || b, b = void 0);
                for (n in a) this.on(n, b, j, a[n], d);
                return this
            }
            null == j && null == c ? (c = b, j = b = void 0) : null == c && ("string" === typeof b ? (c = j, j = void 0) : (c = j, j = b, b = void 0));
            if (!1 === c) c = t;
            else if (!c) return this;
            1 === d && (e = c, c = function(a) {
                g().off(a);
                return e.apply(this, arguments)
            }, c.guid = e.guid || (e.guid = g.guid++));
            return this.each(function() {
                g.event.add(this, a, c, j, b)
            })
        },
        one: function(a, b, g, j) {
            return this.on(a, b, g, j, 1)
        },
        off: function(a, b, j) {
            var c;
            if (a && a.preventDefault && a.handleObj) return c = a.handleObj, g(a.delegateTarget).off(c.namespace ? c.origType + "." + c.namespace : c.origType, c.selector, c.handler), this;
            if ("object" === typeof a) {
                for (c in a) this.off(c, b, a[c]);
                return this
            }
            if (!1 === b || "function" === typeof b) j = b, b = void 0;
            !1 === j && (j = t);
            return this.each(function() {
                g.event.remove(this, a, j, b)
            })
        },
        ...
        triggerHandler: function(a, b) {
            var j = this[0];
            if (j) return g.event.trigger(a, b, j, !0)
        }
    });
    var mc = "abbr|article|aside|audio|bdi|canvas|data|datalist|details|figcaption|figure|footer|header|hgroup|mark|meter|nav|output|progress|section|summary|time|video",
        ...
        ac =
        C(y).appendChild(y.createElement("div"));
    ja.optgroup = ja.option;
    ja.tbody = ja.tfoot = ja.colgroup = ja.caption = ja.thead;
    ja.th = ja.td;
    g.extend({
        clone: function(a, b, j) {
            var c, d, n, e, r, k = g.contains(a.ownerDocument, a);
            v.html5Clone || g.isXMLDoc(a) || !Dc.test("<" + a.nodeName + ">") ? n = a.cloneNode(!0) : (ac.innerHTML = a.outerHTML, ac.removeChild(n = ac.firstChild));
            if ((!v.noCloneEvent || !v.noCloneChecked) && (1 === a.nodeType || 11 === a.nodeType) && !g.isXMLDoc(a)) {
                c = K(n);
                r = K(a);
                for (e = 0; null != (d = r[e]); ++e)
                    if (c[e]) {
                        ...
                        if (1 === l.nodeType) {
                            ...
                            if (!v.noCloneEvent && l[g.expando]) {
                                B = g._data(l);
                                for (t in B.events) g.removeEvent(l, t, B.handle);
                                l.removeAttribute(g.expando)
                            }
                            if ("script" === h && l.text !== d.text) Ea(l).text = d.text, U(l);
                            else if ("object" === h) l.parentNode && (l.outerHTML = d.outerHTML), v.html5Clone && (d.innerHTML && !g.trim(l.innerHTML)) && (l.innerHTML = d.innerHTML);
                            else if ("input" === h && Mb.test(d.type)) l.defaultChecked = l.checked = d.checked, l.value !== d.value && (l.value = d.value);
                            else if ("option" ===
                                h) l.defaultSelected = l.selected = d.defaultSelected;
                            else if ("input" === h || "textarea" === h) l.defaultValue = d.defaultValue
                        }
                    }
            }
            if (b)
                if (j) {
                    r = r || K(a);
                    c = c || K(n);
                    for (e = 0; null != (d = r[e]); e++) Fa(d, c[e])
                } else Fa(a, n);
            c = K(n, "script");
            0 < c.length && P(c, !k && K(a, "script"));
            return n
        },
        buildFragment: function(a, b, j, c) {
            for (var d, n, e, r, k, l, h = a.length, t = C(b), B = [], z = 0; z < h; z++)
                if ((n = a[z]) || 0 === n)
                    if ("object" === g.type(n)) g.merge(B, n.nodeType ? [n] : n);
                    else if (Ad.test(n)) {
                e = e || t.appendChild(b.createElement("div"));
                r = (Fc.exec(n) || ["", ""])[1].toLowerCase();
                l = ja[r] || ja._default;
                e.innerHTML = l[1] + n.replace(Ec, "<$1></$2>") + l[2];
                for (d = l[0]; d--;) e = e.lastChild;
                !v.leadingWhitespace && $b.test(n) && B.push(b.createTextNode($b.exec(n)[0]));
                if (!v.tbody)
                    for (d = (n = "table" === r && !Gc.test(n) ? e.firstChild : "<table>" === l[1] && !Gc.test(n) ? e : 0) && n.childNodes.length; d--;) g.nodeName(k = n.childNodes[d], "tbody") && !k.childNodes.length && n.removeChild(k);
                g.merge(B, e.childNodes);
                for (e.textContent = ""; e.firstChild;) e.removeChild(e.firstChild);
                e = t.lastChild
            } else B.push(b.createTextNode(n));
            e && t.removeChild(e);
            v.appendChecked || g.grep(K(B, "input"), rb);
            for (z = 0; n = B[z++];)
                if (!(c && -1 !== g.inArray(n, c)) && (a = g.contains(n.ownerDocument, n), e = K(t.appendChild(n), "script"), a && P(e), j))
                    for (d = 0; n = e[d++];) Hc.test(n.type || "") && j.push(n);
            return t
        },
        cleanData: function(a, b) {
            for (var c, d, n, e, r = 0, k = g.expando, l = g.cache, h = v.deleteExpando, t = g.event.special; null != (c = a[r]); r++)
                if (b || g.acceptData(c))
                    if (e = (n = c[k]) && l[n]) {
                        if (e.events)
                            for (d in e.events) t[d] ? g.event.remove(c, d) : g.removeEvent(c, d, e.handle);
                        l[n] && (delete l[n],
                            h ? delete c[k] : typeof c.removeAttribute !== ka ? c.removeAttribute(k) : c[k] = null, j.push(n))
                    }
        }
    });
    g.fn.extend({
        ...
        remove: function(a, b) {
            for (var j, c = a ? g.filter(a, this) : this, d = 0; null != (j = c[d]); d++) !b && 1 === j.nodeType && g.cleanData(K(j)), j.parentNode && (b && g.contains(j.ownerDocument,
                j) && P(K(j, "script")), j.parentNode.removeChild(j));
            return this
        },
        empty: function() {
            for (var a, b = 0; null != (a = this[b]); b++) {
                for (1 === a.nodeType && g.cleanData(K(a, !1)); a.firstChild;) a.removeChild(a.firstChild);
        ...
        html: function(a) {
            return Va(this, function(a) {
                ...
                if ("string" === typeof a && !Bd.test(a) && (v.htmlSerialize || !Dc.test(a)) && (v.leadingWhitespace || !$b.test(a)) && !ja[(Fc.exec(a) || ["", ""])[1].toLowerCase()]) {
                    a = a.replace(Ec, "<$1></$2>");
                    try {
                        for (; b < j; b++) f = this[b] || {}, 1 === f.nodeType && (g.cleanData(K(f, !1)), f.innerHTML = a);
                        f = 0
                    } catch (c) {}
                }
        ...
        domManip: function(a, b) {
            ...
            var j, c, d, n, e = 0,
                ...
                t = a[0],
                B = g.isFunction(t);
            if (B || 1 < k && "string" === typeof t && !v.checkClone && Cd.test(t)) return this.each(function(j) {
                var g = l.eq(j);
                B && (a[0] = t.call(this, j, g.html()));
                g.domManip(a, b)
            });
            if (k && (n = g.buildFragment(a, this[0].ownerDocument, !1, this), j = n.firstChild, 1 === n.childNodes.length && (n = j), j)) {
                d = g.map(K(n, "script"), Ea);
                for (c = d.length; e < k; e++) j = n, e !==
                    h && (j = g.clone(j, !0, !0), c && g.merge(d, K(j, "script"))), b.call(this[e], j, e);
                if (c) {
                    n = d[d.length - 1].ownerDocument;
                    g.map(d, U);
                    for (e = 0; e < c; e++)
                        if (j = d[e], Hc.test(j.type || "") && !g._data(j, "globalEval") && g.contains(n, j)) j.src ? g._evalUrl && g._evalUrl(j.src) : g.globalEval((j.text || j.textContent || j.innerHTML || "").replace(Dd, ""))
                }
                n = j = null
            }
            return this
    ...
    g.each({
        ...
        g.fn[a] = function(a) {
            for (var f = 0, j = [],
                    c = g(a), d = c.length - 1; f <= d; f++) a = f === d ? this : this.clone(!0), g(c[f])[b](a), z.apply(j, a.get());
    ...
    v.shrinkWrapBlocks = function() {
        ...
        if (null == Cb) {
            ...
            a.appendChild(b).appendChild(j);
            Cb = !1;
            typeof j.style.zoom !== ka && (j.style.cssText = "-webkit-box-sizing:content-box;-moz-box-sizing:content-box;box-sizing:content-box;display:block;padding:0;margin:0;border:0;width:1px;padding:1px;zoom:1", j.innerHTML = "<div></div>", j.firstChild.style.width = "5px", Cb = 3 !== j.offsetWidth);
            a.removeChild(b)
        }
        return Cb
    };
    ...
    a.getComputedStyle ? (Ra = function(a) {
        ...
        var c, d, n = a.style;
        d = (j = j || Ra(a)) ? j.getPropertyValue(b) || j[b] : void 0;
        j && ("" === d && !g.contains(a.ownerDocument, a) && (d = g.style(a, b)), sb.test(d) && Ic.test(b) && (a = n.width, b = n.minWidth, c = n.maxWidth, n.minWidth = n.maxWidth = n.width = d, d = j.width, n.width = a, n.minWidth = b, n.maxWidth =
            c));
        return void 0 === d ? d : d + ""
        ...
        var g, c, d, n = a.style;
        d = (j = j || Ra(a)) ? j[b] : void 0;
        null == d && (n && n[b]) && (d = n[b]);
        if (sb.test(d) && !Ed.test(b)) {
            j = n.left;
            if (c = (g = a.runtimeStyle) && g.left) g.left = a.currentStyle.left;
    ...
    var dc = function() {
            var f, b, j = y.getElementsByTagName("body")[0];
            j && (f = y.createElement("div"), b = y.createElement("div"),
                f.style.cssText = bc, j.appendChild(f).appendChild(b), b.style.cssText = "-webkit-box-sizing:border-box;-moz-box-sizing:border-box;box-sizing:border-box;position:absolute;display:block;padding:1px;border:1px;width:4px;margin-top:1%;top:1%", g.swap(j, null != j.style.zoom ? {
                    zoom: 1
                } : {}, function() {
                    cc = 4 === b.offsetWidth
                }), Db = !0, Eb = !1, Fb = !0, a.getComputedStyle && (Eb = "1%" !== (a.getComputedStyle(b, null) || {}).top, Db = "4px" === (a.getComputedStyle(b, null) || {
                    width: "4px"
                }).width), j.removeChild(f), b = j = null)
        },
        pb, Gb, cc, Db, Eb, Fb,
    ...
    g.extend(v, {
        reliableHiddenOffsets: function() {
            ...
            if (g) return b.setAttribute("className", "t"), b.innerHTML = "  <link/><table></table><a href='/a'>a</a><input type='checkbox'/>", a = y.createElement("div"), a.style.cssText = bc, g.appendChild(a).appendChild(b), b.innerHTML = "<table><tr><td></td><td>t</td></tr></table>", b = b.getElementsByTagName("td"), b[0].style.cssText = "padding:0;margin:0;border:0;display:none", j = 0 === b[0].offsetHeight, b[0].style.display = "", b[1].style.display =
                "none", Gb = j && 0 === b[0].offsetHeight, g.removeChild(a), Gb
        },
        boxSizing: function() {
            null == cc && dc();
            return cc
        ...
        reliableMarginRight: function() {
            ...
            if (null == Fb && a.getComputedStyle) {
                ...
                j = y.createElement("div");
                b.style.cssText = bc;
                f.appendChild(b).appendChild(j);
                g = j.appendChild(y.createElement("div"));
                g.style.cssText = j.style.cssText =
    ...
    g.swap = function(a, b, j, g) {
        var c, d = {};
        for (c in b) d[c] = a.style[c], a.style[c] = b[c];
        j = j.apply(a, g || []);
        for (c in b) a.style[c] = d[c];
        return j
    };
    var ec = /alpha\([^)]*\)/i,
        Fd = /opacity\s*=\s*([^)]*)/,
    ...
    g.extend({
        ...
        style: function(a, b, j, c) {
            if (a && !(3 === a.nodeType ||
                    8 === a.nodeType || !a.style)) {
                var d, n, e, r = g.camelCase(b),
                    k = a.style;
                b = g.cssProps[r] || (g.cssProps[r] = gb(k, r));
                e = g.cssHooks[b] || g.cssHooks[r];
                if (void 0 !== j) {
                    n = typeof j;
                    if ("string" === n && (d = Hd.exec(j))) j = (d[1] + 1) * d[2] + parseFloat(g.css(a, b)), n = "number";
                    if (!(null == j || j !== j))
                        if ("number" === n && !g.cssNumber[r] && (j += "px"), !v.clearCloneStyle && ("" === j && 0 === b.indexOf("background")) && (k[b] = "inherit"), !e || !("set" in e) || void 0 !== (j = e.set(a, j, c))) try {
                            k[b] = "", k[b] = j
                        } catch (l) {}
                } else return e && "get" in e && void 0 !== (d = e.get(a, !1, c)) ? d : k[b]
            }
        },
        css: function(a, b, j, c) {
            var d, n;
            n = g.camelCase(b);
            b = g.cssProps[n] || (g.cssProps[n] = gb(a.style, n));
            (n = g.cssHooks[b] || g.cssHooks[n]) && "get" in n && (d = n.get(a, !0, j));
            void 0 === d && (d = Sa(a, b, c));
            "normal" === d && b in Jc && (d = Jc[b]);
            return "" === j || j ? (a = parseFloat(d), !0 === j || g.isNumeric(a) ? a || 0 : d) : d
        }
    });
    g.each(["height", "width"], function(a, b) {
        g.cssHooks[b] = {
            ...
            set: function(a,
                f, j) {
                var c = j && Ra(a);
                return la(a, f, j ? x(a, b, j, v.boxSizing() && "border-box" === g.css(a, "boxSizing", !1, c), c) : 0)
            }
        }
    });
    v.opacity || (g.cssHooks.opacity = {
        ...
        set: function(a, b) {
            var j = a.style,
                ...
                n = c && c.filter || j.filter || "";
            j.zoom = 1;
            if ((1 <= b || "" === b) && "" === g.trim(n.replace(ec, "")) && j.removeAttribute)
                if (j.removeAttribute("filter"),
                    "" === b || c && !c.filter) return;
            j.filter = ec.test(n) ? n.replace(ec, d) : n + " " + d
        }
    });
    g.cssHooks.marginRight = fb(v.reliableMarginRight, function(a, b) {
    ...
    g.each({
        ...
        g.cssHooks[a + b] = {
            expand: function(j) {
                var c = 0,
                    g = {};
                for (j = "string" === typeof j ? j.split(" ") : [j]; 4 > c; c++) g[a + Qa[c] + b] = j[c] || j[c - 2] || j[0];
                return g
            }
        };
        Ic.test(a) || (g.cssHooks[a + b].set = la)
    });
    g.fn.extend({
        css: function(a, b) {
            return Va(this, function(a,
                ...
                if (g.isArray(f)) {
                    b = Ra(a);
                    for (j = f.length; d < j; d++) c[f[d]] = g.css(a, f[d], !1, b);
                    return c
                }
    ...
    L.prototype = {
        ...
        run: function(a) {
            ...
            j && j.set ? j.set(this) : L.propHooks._default.set(this);
            return this
        }
    };
    ...
    var Za, Hb, Jd = /^(?:toggle|show|hide)$/,
        ...
        Kd = /queueHooks$/,
        tb = [function(a, b, j) {
            var c, d, n, e, r, k, l = this,
                h = {},
                t = a.style,
                B = a.nodeType && hb(a),
                z = g._data(a, "fxshow");
            j.queue || (e = g._queueHooks(a, "fx"), null == e.unqueued && (e.unqueued = 0, r = e.empty.fire, e.empty.fire = function() {
                    e.unqueued || r()
                }), e.unqueued++,
                l.always(function() {
                    l.always(function() {
            ...
            if (1 === a.nodeType && ("height" in b || "width" in b)) j.overflow = [t.overflow, t.overflowX, t.overflowY], d = g.css(a, "display"), k = db(a.nodeName), "none" === d && (d = k), "inline" === d && "none" === g.css(a, "float") && (!v.inlineBlockNeedsLayout || "inline" === k ? t.display = "inline-block" : t.zoom = 1);
            j.overflow && (t.overflow = "hidden", v.shrinkWrapBlocks() || l.always(function() {
                t.overflow = j.overflow[0];
                t.overflowX = j.overflow[1];
                t.overflowY =
                    j.overflow[2]
            }));
            for (c in b)
                if (d = b[c], Jd.exec(d)) {
                    ...
                    n = n || "toggle" === d;
                    if (d === (B ? "hide" : "show"))
                        if ("show" === d && z && void 0 !== z[c]) B = !0;
                        else continue;
                    h[c] = z && z[c] || g.style(a, c)
                }
            if (!g.isEmptyObject(h))
                for (c in z ? "hidden" in z && (B = z.hidden) : z = g._data(a, "fxshow", {}), n && (z.hidden = !B), B ? g(a).show() : l.done(function() {
                        g(a).hide()
                    }), l.done(function() {
                        var b;
                        g._removeData(a, "fxshow");
                        for (b in h) g.style(a, b, h[b])
                    }), h) b = wa(B ? z[c] : 0, c, l), c in z || (z[c] = b.start, B && (b.end = b.start, b.start = "width" === c || "height" ===
                    c ? 1 : 0))
        }],
        ib = {
            "*": [function(a, b) {
                var j = this.createTween(a, b),
                    c = j.cur(),
                    d = Kc.exec(b),
                    n = d && d[3] || (g.cssNumber[a] ? "" : "px"),
                    e = (g.cssNumber[a] || "px" !== n && +c) && Kc.exec(g.css(j.elem, a)),
                    r = 1,
                    k = 20;
                if (e && e[3] !== n) {
                    n = n || e[3];
                    d = d || [];
                    e = +c || 1;
                    do r = r || ".5", e /= r, g.style(j.elem, a, e + n); while (r !== (r = j.cur() / c) && 1 !== r && --k)
                }
                d && (e = j.start = +e || +c || 0, j.unit = n, j.end = d[1] ? e + (d[1] + 1) * d[2] : +d[2]);
                return j
            }]
        };
    g.Animation = g.extend(jb, {
        tweener: function(a, b) {
            ...
            for (var j, c = 0, d = a.length; c <
                d; c++) j = a[c], ib[j] = ib[j] || [], ib[j].unshift(b)
        },
        prefilter: function(a, b) {
            b ? tb.unshift(a) : tb.push(a)
        }
    });
    g.speed = function(a, b, j) {
        var c = a && "object" === typeof a ? g.extend({}, a) : {
            complete: j || !j && b || g.isFunction(a) && a,
            duration: a,
            easing: j && b || b && !g.isFunction(b) && b
        };
        c.duration = g.fx.off ? 0 : "number" === typeof c.duration ? c.duration : c.duration in g.fx.speeds ? g.fx.speeds[c.duration] : g.fx.speeds._default;
        if (null == c.queue || !0 === c.queue) c.queue = "fx";
        c.old = c.complete;
        c.complete = function() {
            g.isFunction(c.old) && c.old.call(this);
            c.queue && g.dequeue(this, c.queue)
        };
        return c
    };
    g.fn.extend({
        fadeTo: function(a, b, j, c) {
            return this.filter(hb).css("opacity", 0).show().end().animate({
                opacity: b
            }, a, j, c)
        },
        animate: function(a, b, j, c) {
        ...
        stop: function(a, b, j) {
            ...
            "string" !== typeof a && (j = b, b = a, a = void 0);
            b &&
                !1 !== a && this.queue(a || "fx", []);
            return this.each(function() {
                var b = !0,
                    ...
                    n = g.timers,
                    e = g._data(this);
                if (d) e[d] && e[d].stop && c(e[d]);
                else
                    for (d in e) e[d] && (e[d].stop && Kd.test(d)) && c(e[d]);
                for (d = n.length; d--;)
                    if (n[d].elem === this && (null == a || n[d].queue === a)) n[d].anim.stop(j), b = !1, n.splice(d, 1);
                (b || !j) && g.dequeue(this, a)
            })
        },
        finish: function(a) {
            !1 !== a && (a = a || "fx");
            return this.each(function() {
                var b, j = g._data(this),
                    c = j[a + "queue"];
                b = j[a + "queueHooks"];
                var d = g.timers,
                    n = c ? c.length : 0;
                ...
                for (b = 0; b < n; b++) c[b] && c[b].finish && c[b].finish.call(this);
                delete j.finish
            })
        }
    });
    ...
    g.each({
        ...
        g.fn[a] = function(a, f, j) {
            return this.animate(b, a, f, j)
        }
    });
    g.timers = [];
    g.fx.tick = function() {
        var a, b = g.timers,
            j = 0;
        for (Za = g.now(); j < b.length; j++) a = b[j], !a() && b[j] === a && b.splice(j--, 1);
        b.length || g.fx.stop();
        Za = void 0
    };
    ...
    g.fn.delay = function(a, b) {
        ...
        return this.queue(b || "fx", function(b, j) {
    ...
    v.input = "" === Ma.getAttribute("value");
    Ma.value = "t";
    Ma.setAttribute("type", "radio");
    v.radioValue = "t" === Ma.value;
    var Ld = /\r/g;
    g.fn.extend({
        val: function(a) {
            var b, j, c, d = this[0];
            if (arguments.length) return c =
                g.isFunction(a), this.each(function(j) {
                    if (1 === this.nodeType && (j = c ? a.call(this, j, g(this).val()) : a, null == j ? j = "" : "number" === typeof j ? j += "" : g.isArray(j) && (j = g.map(j, function(a) {
                            return null == a ? "" : a + ""
                        })), b = g.valHooks[this.type] || g.valHooks[this.nodeName.toLowerCase()], !b || !("set" in b) || void 0 === b.set(this, j, "value"))) this.value = j
                });
            if (d) {
                if ((b = g.valHooks[d.type] || g.valHooks[d.nodeName.toLowerCase()]) && "get" in b && void 0 !== (j = b.get(d, "value"))) return j;
                j = d.value;
                return "string" === typeof j ? j.replace(Ld, "") :
                    null == j ? "" : j
            }
        }
    });
    g.extend({
        valHooks: {
            ...
            select: {
                get: function(a) {
                    for (var b, j = a.options, c = a.selectedIndex, d = (a = "select-one" === a.type || 0 > c) ? null : [], n = a ? c + 1 : j.length, e = 0 > c ? n : a ? c : 0; e < n; e++)
                        if (b = j[e], (b.selected || e === c) && (v.optDisabled ? !b.disabled : null === b.getAttribute("disabled")) && (!b.parentNode.disabled || !g.nodeName(b.parentNode, "optgroup"))) {
                            b = g(b).val();
                            if (a) return b;
                            d.push(b)
                        }
                ...
                set: function(a, b) {
                    for (var j, c, d = a.options,
                            n = g.makeArray(b), e = d.length; e--;)
                        if (c = d[e], 0 <= g.inArray(g.valHooks.option.get(c), n)) try {
                            c.selected = j = !0
                        } catch (r) {
                            c.scrollHeight
                        } else c.selected = !1;
    ...
    g.each(["radio", "checkbox"], function() {
        ...
        v.checkOn || (g.valHooks[this].get = function(a) {
            return null === a.getAttribute("value") ? "on" : a.value
        })
    });
    var cb, Lc, Na = g.expr.attrHandle,
        hc = /^(?:checked|selected)$/i,
        Wa = v.getSetAttribute,
        Jb = v.input;
    g.fn.extend({
        attr: function(a, b) {
    ...
    g.extend({
        attr: function(a, b, j) {
            var c, d, n = a.nodeType;
            if (a && !(3 === n || 8 === n || 2 === n)) {
                if (typeof a.getAttribute === ka) return g.prop(a, b, j);
                if (1 !== n || !g.isXMLDoc(a)) b = b.toLowerCase(), c = g.attrHooks[b] || (g.expr.match.bool.test(b) ? Lc : cb);
                if (void 0 !== j)
                    if (null === j) g.removeAttr(a, b);
                    else {
                        if (c && "set" in c && void 0 !== (d = c.set(a, j, b))) return d;
                        a.setAttribute(b,
                            j + "");
                        return j
                    }
                else {
                    if (c && "get" in c && null !== (d = c.get(a, b))) return d;
                    d = g.find.attr(a, b);
        ...
        removeAttr: function(a, b) {
            var j, c, d = 0,
                n = b && b.match(xa);
            if (n && 1 === a.nodeType)
                for (; j = n[d++];) c = g.propFix[j] || j, g.expr.match.bool.test(j) ? Jb && Wa || !hc.test(j) ? a[c] = !1 : a[g.camelCase("default-" + j)] = a[c] = !1 : g.attr(a, j, ""), a.removeAttribute(Wa ? j : c)
        },
        attrHooks: {
            type: {
                set: function(a, b) {
                    if (!v.radioValue && "radio" === b && g.nodeName(a, "input")) {
                        var j = a.value;
    ...
    Lc = {
        set: function(a, b, j) {
            !1 === b ? g.removeAttr(a, j) : Jb && Wa || !hc.test(j) ? a.setAttribute(!Wa && g.propFix[j] || j, j) : a[g.camelCase("default-" + j)] = a[j] = !0;
            return j
        }
    };
    g.each(g.expr.match.bool.source.match(/\w+/g), function(a, b) {
        var j = Na[b] || g.find.attr;
        Na[b] = Jb && Wa || !hc.test(b) ? function(a, b, f) {
            var c, g;
            f || (g = Na[b], Na[b] = c, c = null != j(a, b, f) ? b.toLowerCase() : null, Na[b] = g);
            return c
        } : function(a, b, f) {
    ...
    if (!Jb || !Wa) g.attrHooks.value = {
        set: function(a, b, j) {
            ...
            else return cb && cb.set(a, b, j)
        }
    };
    Wa || (cb = {
        set: function(a, b, j) {
            var c = a.getAttributeNode(j);
            c || a.setAttributeNode(c = a.ownerDocument.createAttribute(j));
            c.value = b += "";
            if ("value" === j || b === a.getAttribute(j)) return b
        }
    }, Na.id = Na.name = Na.coords = function(a, b, j) {
        var c;
        if (!j) return (c = a.getAttributeNode(b)) && "" !== c.value ? c.value : null
    }, g.valHooks.button = {
        get: function(a, b) {
            var j = a.getAttributeNode(b);
            if (j && j.specified) return j.value
        },
        set: cb.set
    ...
    g.extend({
        ...
        prop: function(a, b, j) {
            ...
            if (a && !(3 === n || 8 === n || 2 === n)) {
                ...
                return void 0 !== j ? d && "set" in d && void 0 !== (c = d.set(a, j, b)) ? c : a[b] = j : d && "get" in d && null !== (c = d.get(a, b)) ? c : a[b]
            }
        },
        propHooks: {
            tabIndex: {
    ...
    g.fn.extend({
        addClass: function(a) {
            ...
            if (b)
                for (b = (a || "").match(xa) || []; e < r; e++)
                    if (j = this[e], c = 1 === j.nodeType && (j.className ? (" " + j.className + " ").replace(ic, " ") : " ")) {
                        for (n = 0; d = b[n++];) 0 > c.indexOf(" " + d + " ") && (c += d + " ");
                        c = g.trim(c);
                        j.className !== c && (j.className = c)
                    }
            return this
        },
        ...
        toggleClass: function(a, b) {
            var j = typeof a;
            return "boolean" === typeof b && "string" === j ? b ? this.addClass(a) : this.removeClass(a) : g.isFunction(a) ? this.each(function(j) {
                    g(this).toggleClass(a.call(this, j, this.className, b), b)
                }) :
                this.each(function() {
                    if ("string" === j)
                        for (var b, c = 0, d = g(this), n = a.match(xa) || []; b = n[c++];) d.hasClass(b) ? d.removeClass(b) : d.addClass(b);
                    else if (j === ka || "boolean" === j) this.className && g._data(this, "__className__", this.className), this.className = this.className || !1 === a ? "" : g._data(this, "__className__") || ""
                })
        },
        hasClass: function(a) {
            a = " " + a + " ";
            for (var b = 0, j = this.length; b < j; b++)
    ...
    g.fn.extend({
        ...
        bind: function(a, b, j) {
            return this.on(a, null, b, j)
        },
        unbind: function(a, b) {
            return this.off(a, null, b)
        },
        delegate: function(a, b, j, c) {
            return this.on(b, a, j, c)
        },
        undelegate: function(a, b, j) {
            return 1 === arguments.length ? this.off(a, "**") : this.off(b, a || "**", j)
        }
    });
    var jc = g.now(),
        kc = /\?/,
    ...
    g.parseXML = function(b) {
        ...
        try {
            a.DOMParser ? (c = new DOMParser, j = c.parseFromString(b, "text/xml")) : (j = new ActiveXObject("Microsoft.XMLDOM"), j.async = "false", j.loadXML(b))
        } catch (d) {
            j = void 0
        }(!j || !j.documentElement ||
            j.getElementsByTagName("parsererror").length) && g.error("Invalid XML: " + b);
        return j
    };
    var Xa, Oa, Pd = /#.*$/,
        Mc = /([?&])_=[^&]*/,
        ...
        ajax: function(a, b) {
            function j(a, b, f, c) {
                ...
                if (2 !== da) {
                    ...
                    if (f) {
                        p = h;
                        for (var w = s, N, ha, G, C, Q = p.contents, A = p.dataTypes;
                            "*" === A[0];) A.shift(), void 0 === ha && (ha = p.mimeType || w.getResponseHeader("Content-Type"));
                        if (ha)
                            for (C in Q)
                                if (Q[C] && Q[C].test(ha)) {
                                    A.unshift(C);
                        ...
                        G ? (G !== A[0] && A.unshift(G), p = f[G]) : p = void 0
                    }
                    a: {
                        f = h;N = p;ha = s;G = c;
                        var K, v, ua, w = {},
                            Q = f.dataTypes.slice();
                        if (Q[1])
                            for (v in f.converters) w[v.toLowerCase()] = f.converters[v];
                        for (C = Q.shift(); C;)
                            if (f.responseFields[C] && (ha[f.responseFields[C]] = N), !ua && (G && f.dataFilter) && (N = f.dataFilter(N, f.dataType)), ua = C, C = Q.shift())
                                if ("*" === C) C = ua;
                                else if ("*" !== ua && ua !== C) {
                            v = w[ua + " " + C] || w["* " + C];
                            if (!v)
                                for (K in w)
                                    if (p = K.split(" "), p[1] === C && (v = w[ua + " " + p[0]] || w["* " + p[0]])) {
                                        !0 === v ? v =
                                            w[K] : !0 !== w[K] && (C = p[0], Q.unshift(p[1]));
                                        break
                                    }
                            if (!0 !== v)
                                if (v && f["throws"]) N = v(N);
                                    ...
                                    p = {
                                        state: "parsererror",
                                        error: v ? u : "No conversion from " + ua + " to " + C
                                    };
                                    break a
                                }
                        }
                    ...
                    if (c) h.ifModified && ((F = s.getResponseHeader("Last-Modified")) && (g.lastModified[n] = F), (F = s.getResponseHeader("etag")) && (g.etag[n] = F)), 204 === a || "HEAD" === h.type ? F = "nocontent" : 304 === a ? F = "notmodified" : (F = p.state, d = p.data, t = p.error, c = !t);
                    else if (t = F, a || !F) F = "error", 0 > a && (a = 0);
                    s.status = a;
                    s.statusText =
                        (b || F) + "";
                    c ? T.resolveWith(B, [d, F, s]) : T.rejectWith(B, [s, F, t]);
                    s.statusCode(m);
                    m = void 0;
                    k && z.trigger(c ? "ajaxSuccess" : "ajaxError", [s, h, c ? d : t]);
                    q.fireWith(B, [s, F]);
                    k && (z.trigger("ajaxComplete", [s, h]), --g.active || g.event.trigger("ajaxStop"))
                }
            }
            "object" === typeof a && (b = a, a = void 0);
            b = b || {};
            var c, d, n, e, r, k, l, t, h = g.ajaxSetup({}, b),
                B = h.context || h,
                z = h.context && (B.nodeType || B.jquery) ? g(B) : g.event,
                T = g.Deferred(),
                ...
                s = {
                    ...
                    getResponseHeader: function(a) {
                        ...
                        if (2 === da) {
                            if (!t)
                                for (t = {}; b = Qd.exec(e);) t[b[1].toLowerCase()] = b[2];
                            b = t[a.toLowerCase()]
                        }
            ...
            s.success = s.done;
            s.error = s.fail;
            h.url = ((a || h.url || Oa) + "").replace(Pd, "").replace(Sd, Xa[1] + "//");
            h.type = b.method || b.type || h.method || h.type;
            h.dataTypes = g.trim(h.dataType || "*").toLowerCase().match(xa) || [""];
            null == h.crossDomain && (c = Nc.exec(h.url.toLowerCase()), h.crossDomain = !(!c || !(c[1] !== Xa[1] || c[2] !== Xa[2] || (c[3] || ("http:" === c[1] ? "80" : "443")) !== (Xa[3] || ("http:" === Xa[1] ? "80" : "443")))));
            h.data && (h.processData && "string" !== typeof h.data) && (h.data = g.param(h.data, h.traditional));
            ub(Oc, h, b, s);
            if (2 ===
                da) return s;
            (k = h.global) && 0 === g.active++ && g.event.trigger("ajaxStart");
            h.type = h.type.toUpperCase();
            h.hasContent = !Rd.test(h.type);
            n = h.url;
            h.hasContent || (h.data && (n = h.url += (kc.test(n) ? "&" : "?") + h.data, delete h.data), !1 === h.cache && (h.url = Mc.test(n) ? n.replace(Mc, "$1_=" + jc++) : n + (kc.test(n) ? "&" : "?") + "_=" + jc++));
            h.ifModified && (g.lastModified[n] && s.setRequestHeader("If-Modified-Since", g.lastModified[n]), g.etag[n] && s.setRequestHeader("If-None-Match", g.etag[n]));
            (h.data && h.hasContent && !1 !== h.contentType || b.contentType) &&
            s.setRequestHeader("Content-Type", h.contentType);
            s.setRequestHeader("Accept", h.dataTypes[0] && h.accepts[h.dataTypes[0]] ? h.accepts[h.dataTypes[0]] + ("*" !== h.dataTypes[0] ? ", " + Pc + "; q=0.01" : "") : h.accepts["*"]);
            for (d in h.headers) s.setRequestHeader(d, h.headers[d]);
            if (h.beforeSend && (!1 === h.beforeSend.call(B, s, h) || 2 === da)) return s.abort();
            ha = "abort";
            for (d in {
                    success: 1,
                    error: 1,
    ...
    g.each(["get", "post"], function(a, b) {
        g[b] = function(a, j, f, c) {
            g.isFunction(j) && (c = c || f, f = j, j = void 0);
            return g.ajax({
                url: a,
                type: b,
                dataType: c,
        ...
        wrapInner: function(a) {
            return g.isFunction(a) ?
                this.each(function(b) {
                    ...
                    var b = g(this),
                        j = b.contents();
                    j.length ? j.wrapAll(a) : b.append(a)
                })
        },
        wrap: function(a) {
            var b = g.isFunction(a);
            return this.each(function(j) {
                g(this).wrapAll(b ? a.call(this, j) : a)
            })
        },
        unwrap: function() {
    ...
    g.param = function(a, b) {
        ...
        if (g.isArray(a) || a.jquery && !g.isPlainObject(a)) g.each(a, function() {
            ...
            for (j in a) ma(j, a[j], b, d);
        return c.join("&").replace(Td, "+")
    };
    g.fn.extend({
        serialize: function() {
        ...
        serializeArray: function() {
            return this.map(function() {
                ...
                var j = g(this).val();
                return null == j ? null : g.isArray(j) ? g.map(j, function(a) {
                    return {
                        name: b.name,
                        value: a.replace(Qc, "\r\n")
                    }
    ...
    g.ajaxSettings.xhr = void 0 === a.ActiveXObject ? D : function() {
    ...
    (Lb = v.ajax = !!Lb) && g.ajaxTransport(function(a) {
        if (!a.crossDomain || v.cors) {
            ...
            return {
                send: function(j, c) {
                    ...
                    !a.crossDomain && !j["X-Requested-With"] && (j["X-Requested-With"] = "XMLHttpRequest");
                    for (d in j) void 0 !== j[d] && n.setRequestHeader(d, j[d] + "");
                    ...
                    b = function(j, d) {
                        ...
                        if (b && (d || 4 === n.readyState))
                            if (delete Kb[e], b = void 0, n.onreadystatechange = g.noop, d) 4 !== n.readyState && n.abort();
                                ...
                                k = {};
                                r = n.status;
                                "string" === typeof n.responseText && (k.text = n.responseText);
                                try {
                                    h = n.statusText
                                } catch (l) {
                                    h = ""
                                }!r && a.isLocal && !a.crossDomain ? r = k.text ? 200 : 404 : 1223 === r && (r = 204)
                            }
                        k && c(r, h, k, n.getAllResponseHeaders())
                    };
    ...
    g.ajaxTransport("script", function(a) {
        if (a.crossDomain) {
            var b,
                j = y.head || g("head")[0] || y.documentElement;
            return {
                send: function(c, g) {
                    ...
                    b.onload = b.onreadystatechange = function(a, j) {
                        if (j || !b.readyState || /loaded|complete/.test(b.readyState)) b.onload = b.onreadystatechange = null, b.parentNode && b.parentNode.removeChild(b), b = null, j || g(200, "success")
                    };
    ...
    g.ajaxPrefilter("json jsonp", function(b, j, c) {
        ...
        if (r || "jsonp" === b.dataTypes[0]) return d = b.jsonpCallback = g.isFunction(b.jsonpCallback) ? b.jsonpCallback() : b.jsonpCallback, r ? b[r] = b[r].replace(lc, "$1" + d) : !1 !== b.jsonp && (b.url += (kc.test(b.url) ? "&" : "?") + b.jsonp + "=" +
            ...
            a[d] = n;
            b[d] && (b.jsonpCallback = j.jsonpCallback, Rc.push(d));
            e && g.isFunction(n) && n(e[0]);
            e = n = void 0
        }), "script"
    });
    g.parseHTML = function(a, b, j) {
        if (!a || "string" !== typeof a) return null;
        "boolean" === typeof b && (j = b, b = !1);
        b = b || y;
        var c = xc.exec(a);
        j = !j && [];
        if (c) return [b.createElement(c[1])];
        c = g.buildFragment([a], b, j);
        j && j.length && g(j).remove();
        return g.merge([],
            c.childNodes)
    };
    var Sc = g.fn.load;
    g.fn.load = function(a, b, j) {
        if ("string" !== typeof a && Sc) return Sc.apply(this, arguments);
        var c, d, n, e = this,
            r = a.indexOf(" ");
        0 <= r && (c = a.slice(r, a.length), a = a.slice(0, r));
        g.isFunction(b) ? (j = b, b = void 0) : b && "object" === typeof b && (n = "POST");
        0 < e.length && g.ajax({
            url: a,
            type: n,
            dataType: "html",
            ...
            d = arguments;
            e.html(c ? g("<div>").append(g.parseHTML(a)).find(c) : a)
        }).complete(j && function(a, b) {
            e.each(j, d || [a.responseText, b, a])
        });
        return this
    };
    ...
    g.offset = {
        setOffset: function(a, b, j) {
            var c, d, n, e = g.css(a, "position"),
                r = g(a),
                h = {};
            ...
            d = g.css(a, "top");
            c = g.css(a, "left");
            ("absolute" === e || "fixed" === e) && -1 < g.inArray("auto", [d, c]) ? (c = r.position(), d = c.top, c = c.left) : (d = parseFloat(d) || 0, c = parseFloat(c) || 0);
            g.isFunction(b) && (b = b.call(a, j, n));
            null != b.top && (h.top = b.top - n.top + d);
            null != b.left && (h.left =
                b.left - n.left + c);
            "using" in b ? b.using.call(a, h) : r.css(h)
        }
    };
    g.fn.extend({
        offset: function(a) {
            ...
            var b, j, c = {
                ...
                d = (j = this[0]) && j.ownerDocument;
            if (d) {
                b = d.documentElement;
                if (!g.contains(b, j)) return c;
                typeof j.getBoundingClientRect !== ka && (c = j.getBoundingClientRect());
                j = V(d);
                return {
                    top: c.top + (j.pageYOffset || b.scrollTop) - (b.clientTop || 0),
                    left: c.left + (j.pageXOffset || b.scrollLeft) - (b.clientLeft || 0)
                }
            }
        },
        position: function() {
            if (this[0]) {
                var a,
                    ...
                    c = this[0];
                "fixed" === g.css(c, "position") ? b = c.getBoundingClientRect() : (a = this.offsetParent(), b = this.offset(), g.nodeName(a[0], "html") || (j = a.offset()), j.top += g.css(a[0], "borderTopWidth", !0), j.left += g.css(a[0], "borderLeftWidth", !0));
                return {
                    top: b.top - j.top - g.css(c, "marginTop", !0),
                    left: b.left - j.left - g.css(c, "marginLeft", !0)
                }
            }
        },
    ...
    g.each({
        ...
        var j = /Y/.test(b);
        g.fn[a] = function(c) {
            return Va(this, function(a, c, f) {
                var d = V(a);
                if (void 0 === f) return d ? b in d ? d[b] : d.document.documentElement[c] : a[c];
                d ? d.scrollTo(!j ? f : g(d).scrollLeft(), j ? f : g(d).scrollTop()) : a[c] = f
            }, a, c, arguments.length, null)
        }
    });
    g.each(["top", "left"], function(a, b) {
        g.cssHooks[b] = fb(v.pixelPosition, function(a, j) {
            if (j) return j = Sa(a, b), sb.test(j) ? g(a).position()[b] + "px" : j
        })
    });
    g.each({
            Height: "height",
        ...
        function(a, b) {
            g.each({
                ...
                g.fn[c] = function(c, d) {
                    var n = arguments.length && (j || "boolean" !== typeof c),
                        e = j || (!0 === c || !0 === d ? "margin" : "border");
                    return Va(this, function(b, j, c) {
                        return g.isWindow(b) ? b.document.documentElement["client" + a] : 9 === b.nodeType ? (j = b.documentElement, Math.max(b.body["scroll" + a], j["scroll" + a], b.body["offset" + a], j["offset" + a], j["client" + a])) : void 0 === c ? g.css(b, j, e) : g.style(b, j, c, e)
                    }, b, n ? c : void 0, n, null)
                }
            })
        });
        ...
        return {
            send: function(k, q) {
                ...
                h.onload = h.onreadystatechange = function(a, e) {
                    if (e || !h.readyState || /loaded|complete/.test(h.readyState)) h.onload = h.onreadystatechange = null, h.parentNode && h.parentNode.removeChild(h), h = null, e || q(200, "success")
                };
    ...
    a.support.iecors ? a.ajaxTransport(function(a) {
        return {
            send: function(h, e) {
                ...
                a.xhrFields && (k.onerror = a.xhrFields.error, k.ontimeout = a.xhrFields.timeout);
                k.open(a.type, a.url);
        ...
        return {
            send: function(e, k) {
                ...
                q.open(a.type, a.url, a.async);
                var m = !1;
                try {
                    q.hasOwnProperty("responseType") && (q.responseType = "arraybuffer", m = !0)
                } catch (u) {}
                try {
                    !m && q.overrideMimeType && q.overrideMimeType("text/plain; charset=x-user-defined")
                } catch (b) {}!a.crossDomain && !e["X-Requested-With"] && (e["X-Requested-With"] = "XMLHttpRequest");
                try {
                    for (var c in e) q.setRequestHeader(c, e[c])
                } catch (d) {}
                ...
                h = function() {
                    ...
                    try {
                        if (h && 4 === q.readyState) {
                            h = void 0;
                            try {
                                e.text = "string" === typeof q.responseText ?
                                    q.responseText : null
                            } catch (m) {}
                            try {
                                e.binary = q.response
                ...
                a.async ? 4 === q.readyState ? setTimeout(h) : q.onreadystatechange = h : h()
            },
            abort: function() {}
        }
    }))
...
Jmol._version || (Jmol = function(a) {
    var m = function(a) {
        ...
        m = {
            _version: "$Date: 2017-07-06 02:22:33 -0500 (Thu, 06 Jul 2017) $",
            _alertNoBinary: !0,
...
(function(a, m) {
    ...
    var k;
    a: {
        k = ["linux", "unix", "mac", "win"];
        for (var q = k.length; q--;)
            if (-1 != e.ua.indexOf(k[q])) {
                k = k[q];
                break a
            }
        k = "unknown"
    ...
    a._getGrabberOptions = function(b) {
        ...
        for (var c = c + '<select id="ID_select">', e = 0; e < a._grabberOptions.length; e++) var h = a._grabberOptions[e],
            c = c + ('<option value="' + h[0] + '" ' + (0 == e ? "selected" : "") + ">" + h[1] + "</option>");
    ...
    a._getDirectDatabaseCall = function(b, c) {
        ...
        var d = 2,
            e = b.substring(0, d),
            h = a.db._DirectDatabaseCalls[e] || a.db._DirectDatabaseCalls[e = b.substring(0, --d)];
        h && (":" == e ? (e = b.toLowerCase(), isNaN(parseInt(b.substring(1))) ? 0 == e.indexOf(":smiles:") ? (h += "?POST?smiles=" + b.substring(8), b = "smiles") : 0 == e.indexOf(":cid:") ? b = "cid/" + b.substring(5) : (0 == e.indexOf(":name:") ? b = b.substring(5) : 0 == e.indexOf(":cas:") && (b = b.substring(4)), b = "name/" + encodeURIComponent(b.substring(d))) :
            b = "cid/" + b.substring(1)) : b = encodeURIComponent(b.substring(d)), 0 <= b.indexOf(".mmtf") ? b = "http://mmtf.rcsb.org/full/" + b.replace(/\.mmtf/, "") : 0 <= h.indexOf("FILENCI") ? (b = b.replace(/\%2F/g, "/"), b = h.replace(/\%FILENCI/, b)) : b = h.replace(/\%FILE/, b));
        return b
    };
    a._getRawDataFromServer = function(b, c, d, e, h, k) {
        b = "?call=getRawDataFromDatabase&database=" + b + (0 <= c.indexOf("?POST?") ? "?POST?" : "") + "&query=" + encodeURIComponent(c) + (h ? "&encoding=base64" : "") + (k ? "" : "&script=" + encodeURIComponent(a._getScriptForDatabase(b)));
    ...
    a._loadFileData = function(b, c, d, e) {
        var h = [];
        c = a._checkFileName(b, c, h);
        d = a._checkCache(b, c, d);
        h[0] ? a._getRawDataFromServer("_", c, d, e) : (b = {
            type: "GET",
            dataType: "text",
            url: c,
    ...
    a._extractInfoFromRCSB = function(b, c, d, e) {
        var h = e.length / 5;
        if (0 != h && 4 == d.length && 1 != h) {
            d = d.toUpperCase();
            var k = e.indexOf(d);
            0 < k && 0 <= "123456789".indexOf(d.substring(0,
                1)) && (e = d + "," + e.substring(0, k) + e.substring(k + 5));
            50 < h && (e = e.substring(0, 250));
            e = e.replace(/\n/g, ",");
    ...
    a._checkAjaxPost = function(a) {
        ...
        0 < c && (a.data = a.url.substring(c + 6), a.url = a.url.substring(0, c), a.type = "POST", a.contentType = "application/x-www-form-urlencoded")
    };
    ...
    a._setQueryTerm = function(b, c) {
        if (c && b._hasOptions && "http://" != c.substring(0, 7)) {
            if (a._isDatabaseCall(c)) {
                ...
                if (e && e.options)
                    for (var h = 0; h < e.options.length; h++) e[h].value == d && (e[h].selected = !0)
            }
            a.$val(a.$(b, "query"), c)
        }
    };
    ...
    a._searchMol = function(b, c, d, e) {
        var h;
        a._isDatabaseCall(c) ? (h = c.substring(0, 1), c = c.substring(1)) : h = b._hasOptions ? a.$val(a.$(b, "select")) : "$";
        "=" == h && 3 == c.length && (c = "=" + c);
        var k = h + c;
        if (c && !(0 > k.indexOf("?") && k == b._thisJmolModel)) {
            b._thisJmolModel = k;
            var l;
            e && null != b._viewSet && null != (l = a.View.__findView(b._viewSet, {
                chemID: k
            })) ? a.View.__setView(l,
                b, !1) : ("$" == h || ":" == h ? b._jmolFileType = "MOL" : "=" == h && (b._jmolFileType = "PDB"), b._searchDatabase(c, h, d))
        }
    };
    ...
    a._canSyncBinary = function(b) {
        ...
        try {
            ...
            return System.out.println("JSmolCore.js: synchronous binary file transfer is requested but not available"), a._alertNoBinary && !b && alert("JSmolCore.js: synchronous binary file transfer is requested but not available"), a._syncBinaryOK = !1
        }
        return !0
    };
    ...
    a._getFileData = function(b, c, d) {
        var e = a._isBinaryUrl(b),
            h = 0 <= b.indexOf(".gz") && 0 <= b.indexOf("rcsb.org");
        h && (b = b.replace(/\.gz/, ""), e = !1);
        var h = e && !c && !a._canSyncBinary(h),
            k = 0 <= b.indexOf("?POST?");
        ...
        if (q || h || !l && !m || !c && s) u = a._getRawDataFromServer("_", b, c, c, h, !0);
            ...
            k ? (P.type = "POST", P.url = b.split("?POST?")[0], P.data = b.split("?POST?")[1]) : (P.type = "GET", P.url = b);
            c && (P.success = function() {
                    c(a._xhrReturn(P.xhr))
                },
                P.error = function() {
                    c(P.xhr.statusText)
                });
            P.xhr = a.$ajax(P);
            c || (u = a._xhrReturn(P.xhr))
        }
        if (!d) return u;
        null == u && (u = "", e = !1);
    ...
    a._loadFileAsynchronously = function(b, c, d, e) {
        ...
        a.$appEvent(c, "localReader_loadfile", "click", function() {
            var d = a.$(c, "localReader_files")[0].files[0],
                h = new FileReader;
            h.onloadend = function(h) {
                h.target.readyState == FileReader.DONE && (a._hideLocalFileReader(c, 0), a._setData(b, d.name, d.name, h.target.result, e, c))
            };
            try {
    ...
    a._setData = function(b, c, d, e, h, k) {
        e && (e = a._strToBytes(e));
        null != e && (null == b || 0 <= c.indexOf(".jdx")) && a.Cache.put("cache://" + c, e);
        null == b ? k._applet.openFileAsyncSpecial(null == e ? c : "cache://" + c, 1) : b.setData(c, d, e, h)
    };
    ...
    a._saveFile = function(b, c, d, e) {
        ...
        a._useDataURI || !h ? (e = document.createElement("a"), e.href = "data:" + d + ";base64," + c, e.type = d || "text/plain;charset=utf-8", e.download = b, e.target = "_blank", m("body").append(e), e.click(), e.remove()) : (a._formdiv || (a.$after("body", '<div id="__jsmolformdiv__" style="display:none">\t\t\t\t\t\t<form id="__jsmolform__" method="post" target="_blank" action="">\t\t\t\t\t\t<input name="call" value="saveFile"/>\t\t\t\t\t\t<input id="__jsmolmimetype__" name="mimetype" value=""/>\t\t\t\t\t\t<input id="__jsmolencoding__" name="encoding" value=""/>\t\t\t\t\t\t<input id="__jsmolfilename__" name="filename" value=""/>\t\t\t\t\t\t<textarea id="__jsmoldata__" name="data"></textarea>\t\t\t\t\t\t</form>\t\t\t\t\t\t</div>'),
            a._formdiv = "__jsmolform__"), a.$attr(a._formdiv, "action", h + "?" + (new Date).getMilliseconds()), a.$val("__jsmoldata__", c), a.$val("__jsmolfilename__", b), a.$val("__jsmolmimetype__", d), a.$val("__jsmolencoding__", e), a.$submit("__jsmolform__"), a.$val("__jsmoldata__", ""), a.$val("__jsmolfilename__", ""));
        return "OK"
    };
    ...
    a._readyCallback = function(b, c, d, e, h) {
        b = b.split("_object")[0];
        var k = a._applets[b];
        if (d = d.booleanValue ? d.booleanValue() : d) k._appletPanel = h || e, k._applet = e;
        a._track(k)._readyCallback(b, c, d)
    };
    a._getWrapper = function(b, c) {
        ...
        if (c) {
            ...
            if (b._coverImage) var e = ' onclick="Jmol.coverApplet(ID, false)" title="' + b._coverTitle + '"',
                h = '<image id="ID_coverclickgo" src="' +
                ...
                e = '<div id="ID_coverdiv" style="background-color:red;z-index:' + a._getZ(b, "coverImage") + ';width:100%;height:100%;display:inline;position:absolute;top:0px;left:0px"><image id="ID_coverimage" src="' + b._coverImage + '" style="width:100%;height:100%"' + e + "/>" + h + "</div>";
            h = b._isJava ? "" : '<image id="ID_waitimage" src="' + b._j2sPath + '/img/cursor_wait.gif" style="display:none;position:absolute;bottom:10px;left:10px;z-index:' +
                a._getZ(b, "coverImage") + ';" />';
            d = a._appletCssText.replace(/\'/g, '"');
            var k = b._getSpinner && b._getSpinner();
            b._spinner = k = !k || "none" == k ? "" : "background-image:url(" + k + "); background-repeat:no-repeat; background-position:center;";
            d = k + (0 <= d.indexOf('style="') ? d.split('style="')[1] : '" ' + d);
            ...
            d = d.replace(/IMG/, e).replace(/WAIT/, h).replace(/Hpx/g, k).replace(/Wpx/g, l)
    ...
    a._sortMessages = function(a) {
        ...
        for (var d = [], e = a.length - 1; 0 <= e; e--)
            for (var h = 0, k = a[e].length; h < k; h++) d[d.length] = a[e][h];
    ...
    a._jsGetXY = function(b, c) {
        ...
        var d = a.$offset(b.id),
            e, h = c.originalEvent;
        c.pageX || (c.pageX = h.pageX);
        c.pageY || (c.pageY = h.pageY);
        a._mousePageX = c.pageX;
        a._mousePageY = c.pageY;
        h.targetTouches && h.targetTouches[0] ? (e = h.targetTouches[0].pageX - d.left, d = h.targetTouches[0].pageY - d.top) : h.changedTouches ? (e = h.changedTouches[0].pageX - d.left, d = h.changedTouches[0].pageY - d.top) : (e = c.pageX - d.left, d = c.pageY - d.top);
        return void 0 ==
        ...
        switch (c.type) {
            ...
            case "touchmove":
                var e = a.$offset(b.id),
                    h = b._touches[0],
                    k = b._touches[1];
                h.push([d.touches[0].pageX - e.left, d.touches[0].pageY - e.top]);
                k.push([d.touches[1].pageX - e.left, d.touches[1].pageY - e.top]);
                d = h.length;
                3 < d && (h.shift(), k.shift());
    ...
    a._jsSetMouse = function(b) {
        ...
        a.$bind(b, "mousedown touchstart", function(d) {
            ...
            var h = a._jsGetXY(b, d);
            h && (2 != d.button && a.Swing.hideMenus(b.applet), b.applet._processEvent(501, h));
            return !!e
        });
        ...
        a._drag = function(b, c) {
            ...
            if (!e) return !1;
            b.isDragging || (e[2] = 0);
            var h = c.target["data-UI"];
            b.isdragging && (!h || h.handleJSEvent(b, 506, c));
        ...
        a.$bind(b, "mousemoveoutjsmol", function(d, e, h) {
            if (c(h)) return !0;
            if (b == a._mouseOwner && b.isDragging) return a._drag(b, h)
        });
    ...
    (function(b) {
        ...
        b.updateFromSync = function(c, d) {
            ...
            if (e) {
                var h = b.__findView(c._viewSet, {
                    viewID: e
                });
                if (null == h) return a.updateView(c, d);
                h != c._currentView && b.__setView(h, c, !0);
                var k = (e = a._getAttr(d, "atoms")) && 0 <= d.indexOf("selectionhalos ON") ? eval("[" + e + "]") : [];
                setTimeout(function() {
                    c._currentView == h && b.updateAtomPick(c, k)
                }, 10);
        ...
        b.updateAtomPick = function(b, d) {
            ...
            if (null != e) {
                for (var h in e) "info" != h && e[h].applet != b && e[h].applet._updateAtomPick(d);
        ...
        b.dumpViews = function(a) {
            ...
            if (d) {
                var e = "View set " + a + ":\n";
                a = b.applets[a];
                for (var h in a) e += "\napplet " + a[h]._id + " currentView=" + (a[h]._currentView ? a[h]._currentView.info.viewID : null);
                for (h = d.length; 0 <= --h;) {
                    a = d[h];
        ...
        b.__findView = function(a, d) {
            var e = b.sets[a];
            null == e && (e = b.sets[a] = []);
            for (var h = e.length; 0 <= --h;) {
                var k = e[h];
                if (d.viewID) {
                    if (k.info.viewID == d.viewID) return k
                } else {
                    if (null != d.chemID && d.chemID == k.info.chemID) return k;
                    for (var l in k)
                        if ("info" != l && (null != d.data && null != k[l].data ? d.data == k[l].data : d.type == l)) return k
                }
            }
            return null
        };
        b.__createViewSet =
            function(c, d, e) {
                ...
                for (var h in a._applets) e = a._applets[h], e._viewSet == c && (d[e._viewType] = {
                    applet: e,
                    data: null
                });
                b.sets[c].push(d);
        ...
        b.__setView = function(a, b, e) {
            for (var h in a)
                if ("info" != h) {
                    var k = a[h],
                        l = k.applet,
                        q = e || null != l && "<modified>" == l._molData;
                    if (!(null == l || l == b && !q)) {
                        var m = null == k.data,
                            s = null != l._currentView;
                        l._currentView = a;
                        if (!s || !(a[h].data == k.data && !m & !q))
    ...
    a.Cache.setDragDrop = function(b) {
        ...
        a.$appEvent(b, "appletdiv", "drop", function(c) {
            ...
            } catch (h) {} else d = new FileReader, d.onloadend = function(d) {
                if (d.target.readyState == FileReader.DONE) {
                    var h = "cache://DROP_" + e.name;
                    d = Clazz.newByteArray(-1, d.target.result);
                    h.endsWith(".spt") || b._appletPanel.cacheFileByName("cache://DROP_*", !1);
                    "JSV" == b._viewType || h.endsWith(".jdx") ? a.Cache.put(h, d) : b._appletPanel.cachePut(h, d);
                    (d = a._jsGetXY(b._canvas, c)) && (!b._appletPanel.setStatusDragDropped || b._appletPanel.setStatusDragDropped(0, d[0], d[1], h)) && b._appletPanel.openFileAsyncSpecial(h,
                        1)
                }
...
(function(a) {
        ...
        s = function(b) {
            ...
            if (0 != h.length)
                if (!a._isAsync && !b) setTimeout(s, 10);
                    ...
                    d.push("done");
                    var k = "JSmol exec " + d[0]._id + " " + d[3] + " " + d[2];
    ...
    a._addCoreFile = function(b, d, e) {
        ...
        if (!(0 <= k.join("").indexOf(b))) {
            k.push(b);
            k.sort();
            a._coreFiles = [d + "/core/core" + k.join("") + ".z.js"];
            if (e && (e = e.split(" ")))
                for (b = 0; b < e.length; b++) 0 > q.join("").indexOf(e[b]) && q.push(d + "/core/core" + e[b] + ".z.js");
            for (b = 0; b < q.length; b++) a._coreFiles.push(q[b])
        }
    };
    a._Canvas2D = function(b, d, e, h) {
    ...
    a._setAppletParams = function(b, d, e, h) {
        for (var k in e)
            if (!b || 0 <= b.indexOf(";" + k.toLowerCase() + ";")) null == e[k] || "language" == k && !a.featureDetection.supportsLocalization() || (h ? d.put(k, !0 === e[k] ? Boolean.TRUE : !1 === e[k] ? Boolean.FALSE : e[k]) : d[k] = e[k])
    };
    a._jsSetPrototype = function(b) {
        ...
        b._createCanvas2d = function() {
            ...
            if (this._isLayered) {
                ...
                this._isSwing ? (b = document.createElement("div"), b.id = this._id + "_swingdiv", a._$(this._id + "_appletinfotablediv").append(b), a._$(b.id).css({
                    ...
                    height: h + "px",
                    overflow: "hidden"
                }), this._mouseInterface = k.contentLayer, k.contentLayer.applet = this) : this._mouseInterface = this._getLayer("front", b, e, h, !1)
            } else this._mouseInterface = k;
            a._jsSetMouse(this._mouseInterface)
        };
        b._getLayer = function(b, c, e, h, k) {
        ...
        b.__startAppletJS = function(b) {
            ...
            var e = b._j2sPath + "/";
            if (0 > e.indexOf("://")) {
                var h = document.location.href.split("#")[0].split("?")[0].split("/");
                0 == e.indexOf("/") ? h = [h[0], e.substring(1)] : h[h.length - 1] = e;
                e = h.join("/")
            }
            c.put("codePath", e);
    ...
    a._repaint = function(b, e) {
        if (b && b._appletPanel) {
            var h = a.$(b, "appletdiv"),
                k = Math.round(h.width()),
                h = Math.round(h.height());
            if (b._is2D && (b._canvas.width !=
                    k || b._canvas.height != h)) b._newCanvas(!0), b._appletPanel.setDisplay(b._canvas);
            b._appletPanel.setScreenDimension(k, h);
            k = function() {
                b._appletPanel.paint ? b._appletPanel.paint(null) : b._appletPanel.update(null)
            };
            e ? requestAnimationFrame(k) : k()
        }
    };
    a._loadImage = function(b, e, h, k, l, q) {
        var m = "echo_" + e + h + (k ? "_" + k.length : ""),
            s = a._getHiddenCanvas(b.vwr.html5Applet, m, 0, 0, !1, !0);
        if (null == s) {
            if (null == q) {
                q = new Image;
                if (null == k) return q.onload = function() {
                    a._loadImage(b, e, h, null, l, q)
                }, q.src = h, null;
                System.out.println("Jsmol.js Jmol._loadImage using data URI for " +
                    m);
                q.src = "string" == typeof k ? k : "data:" + JU.Rdr.guessMimeTypeForBytes(k) + ";base64," + JU.Base64.getBase64(k)
            }
            var u = q.width,
                U = q.height;
            "webgl" == e && (u /= 2, U /= 2);
        ...
        return null == k ? l(s, h) : s
    };
    a._canvasCache = {};
    a._getHiddenCanvas = function(b, e, h, k, l, q) {
        e = b._id + "_" + e;
        b = a._canvasCache[e];
        if (q) return b;
        if (l || !b || b.width !=
            h || b.height != k) b = document.createElement("canvas"), b.width = b.style.width = h, b.height = b.style.height = k, b.id = e, a._canvasCache[e] = b;
        return b
    };
...
(function(a, m) {
    a._Applet = function(e, h, l) {
        window[e] = this;
        this._jmolType = "Jmol._Applet" + (h.isSigned ? " (signed)" : "");
        this._viewType = "Jmol";
        this._isJava = !0;
        this._syncKeyword = "Select:";
        this._availableParams = ";progressbar;progresscolor;boxbgcolor;boxfgcolor;allowjavascript;boxmessage;\t\t\t\t\t\t\t\t\t;messagecallback;pickcallback;animframecallback;appletreadycallback;atommovedcallback;\t\t\t\t\t\t\t\t\t;echocallback;evalcallback;hovercallback;language;loadstructcallback;measurecallback;\t\t\t\t\t\t\t\t\t;minimizationcallback;resizecallback;scriptcallback;statusform;statustext;statustextarea;\t\t\t\t\t\t\t\t\t;synccallback;usecommandthread;syncid;appletid;startupscript;menufile;";
        if (l) return this;
        this._isSigned = h.isSigned;
        this._readyFunction = h.readyFunction;
        this._ready = !1;
        this._isJava = !0;
        this._isInfoVisible = !1;
        this._applet = null;
        this._memoryLimit = h.memoryLimit || 512;
        this._canScript = function() {
            return !0
        };
        this._savedOrientations = [];
        this._initialize = function(e, l) {
            var b = !1;
            a._jarFile && (l = a._jarFile);
            if (this._jarFile) {
                var c = this._jarFile;
                0 <= c.indexOf("/") ? (alert("This web page URL is requesting that the applet used be " + c + ". This is a possible security risk, particularly if the applet is signed, because signed applets can read and write files on your local machine or network."),
                    "yes" == prompt("Do you want to use applet " + c + "? ", "yes or no") ? (e = c.substring(0, c.lastIndexOf("/")), l = c.substring(c.lastIndexOf("/") + 1)) : b = !0) : l = c;
                this_isSigned = h.isSigned = 0 <= l.indexOf("Signed")
            }
            this._jarPath = h.jarPath = e || ".";
            this._jarFile = h.jarFile = "string" == typeof l ? l : (l ? "JmolAppletSigned" : "JmolApplet") + "0.jar";
    ...
    l._get = function(e, h, q) {
        ...
        a._debugAlert =
            h.debug;
        h.serverURL && (a._serverUrl = h.serverURL);
        for (var m = !1, u = null, b = h.use.toUpperCase().split("#")[0].split(" "), c = 0; c < b.length; c++) {
            switch (b[c]) {
                case "JAVA":
                    m = !0;
                    a.featureDetection.supportsJava() && (u = new l(e, h, q));
                    break;
                case "WEBGL":
                    u = l._getCanvas(e, h, q, !0);
                    break;
                case "HTML5":
                    a.featureDetection.allowHTML5 ? u = l._getCanvas(e, h, q, !1) : b.push("JAVA")
            }
            if (null != u) break
        }
        null == u && (q || !m ? u = {
        ...
        return q ? u : a._registerApplet(e, u)
    };
    l._getCanvas = function(e, h, q, m) {
        h._isLayered = !1;
        h._platform = "J.awtjs2d.Platform";
        return m && a.featureDetection.supportsWebGL() ? (a._Canvas3D.prototype = a.GLmol.extendApplet(a._jsSetPrototype(new l(e, h, !0))), new a._Canvas3D(e, h, "Jmol", q)) : !m ? (a._Canvas2D.prototype = a._jsSetPrototype(new l(e, h, !0)), new a._Canvas2D(e, h, "Jmol", q)) : null
    };
    ...
    l._createApplet = function(e, h, q) {
        e._initialize(h.jarPath, h.jarFile);
        var s = e._jarFile;
        ...
        if (0 > q.codePath.indexOf("://")) {
            ...
            c[c.length - 1] = q.codePath;
            q.codePath = c.join("/")
        }
        q.archive = s;
        q.mayscript = "true";
        q.java_arguments = "-Xmx" + Math.round(h.memoryLimit || e._memoryLimit) + "m";
        q.permissions = e._isSigned ? "all-permissions" : "sandbox";
        q.documentLocation = m.location.href;
        q.documentBase = m.location.href.split("#")[0].split("?")[0];
        q.jarPath = h.jarPath;
        a._syncedApplets.length && (q.synccallback = "Jmol._mySyncCallback");
        e._startupScript && (q.script = e._startupScript);
        var c = "\n",
            d;
        for (d in q) q[d] && (c += "  <param name='" + d + "' value='" + q[d] + "' />\n");
        ...
        c = a._getWrapper(e, !0) + c + a._getWrapper(e, !1) + (h.addSelectionOptions ? a._getGrabberOptions(e) : "");
    ...
    h._create = function(e, h) {
        a._setObject(this, e, h);
        var q = {
            syncId: a._syncId,
            progressbar: "true",
            progresscolor: "blue",
        ...
        a._setAppletParams(this._availableParams, q, h);
        var m;
        h.inlineModel ? (m = h.inlineModel, m = m.replace(/\r|\n|\r\n/g, 0 <= m.indexOf("|") ? "\\/n" : "|").replace(/'/g, "&#39;"), a._debugAlert && alert("inline model:\n" + m)) : m = "";
        q.loadInline = m;
        q.appletReadyCallback = "Jmol._readyCallback";
        a._syncedApplets.length && (q.synccallback = "Jmol._mySyncCallback");
        q.java_arguments = "-Xmx" + Math.round(h.memoryLimit || this._memoryLimit) + "m";
        this._initialize(h.jarPath,
            h.jarFile);
        l._createApplet(this, h, q)
    };
    h._restoreState = function(e, h) {
        System.out.println("\n\nasynchronous restore state for " + e + " " + h);
        var l = this,
            m = l._applet && l._applet.viewer;
        switch (h) {
            case "setOptions":
                return function() {
                    l.__startAppletJS(l)
            ...
            default:
                ...
                if (m && m.isScriptExecuting && m.isScriptExecuting()) {
                    if (a._asyncCallbacks[e]) return System.out.println("...ignored"),
                        1;
                    var u = m.getEvalContextAndHoldQueue(m.eval),
                        b = u.pc - 1;
                    u.asyncID = e;
                    a._asyncCallbacks[e] = function(a) {
                        u.pc = a;
                        System.out.println("sc.asyncID=" + u.asyncID + " sc.pc = " + u.pc);
                        m.eval.resumeEval(u)
                    };
                    m.eval.pc = m.eval.pcEnd;
                    System.out.println("setting resume for pc=" + u.pc + " " + e + " to " + a._asyncCallbacks[e] + "//");
                    return function() {
    ...
    h._readyCallback = function(e, h, l) {
        if (l) {
            a._setDestroy(this);
            this._ready = !0;
            e = this._readyScript;
            ...
            a._setReady(this);
            if ((e = this._2dapplet) && e._isEmbedded && e._ready && e.__Info.visible) this._show2d(!0), this._show2d(!1), this._show2d(!0);
    ...
    h._showInfo = function(e) {
        ...
        if (!this._isInfoVisible != !e) {
            ...
            if (this._isJava) {
                var h = e ? 2 : "100%";
                a.$setSize(a.$(this, "appletdiv"), h, h)
            }
    ...
    h._getAtomCorrelation = function(a) {
        ...
        for (var h = this._evaluate("{*}.count"), l = [], m = [], u = 0; u < a.length; u++) {
            var b = a[u];
            l[b[0] + 1] = b[1] - h + 1;
            m[b[1] - h + 1] = b[0] + 1
        }
    ...
    h._scriptWait = function(a, h) {
        var l = this._scriptWaitAsArray(a),
            m = "";
        if (!h)
            for (var u = l.length; 0 <= --u;)
                for (var b = 0, c = l[u].length; b < c; b++) m += l[u][b] + "\n";
        return m
    };
    h._scriptEcho = function(a) {
        a = this._scriptWaitAsArray(a);
        for (var h = "", l = a.length; 0 <= --l;)
            for (var m = a[l].length; 0 <= --m;) "scriptEcho" == a[l][m][1] && (h += a[l][m][3] + "\n");
        return h.replace(/ \| /g, "\n")
    };
    h._scriptMessage = function(a) {
        a =
            this._scriptWaitAsArray(a);
        for (var h = "", l = a.length; 0 <= --l;)
            for (var m = a[l].length; 0 <= --m;) "scriptStatus" == a[l][m][1] && (h += a[l][m][3] + "\n");
        return h.replace(/ \| /g, "\n")
    };
    ...
    h._scriptWaitAsArray = function(e) {
        var h = "";
        try {
            if (this._getStatus("scriptEcho,scriptMessage,scriptStatus,scriptError"), e && (h += this._applet.scriptWait(e), h = a._evalJSON(h, "jmolStatus"), "object" == typeof h)) return h
        } catch (l) {}
        return [
            [h]
        ]
    ...
    h._resizeApplet = function(e) {
        function h(e, l) {
            var b = "" + e;
            return 0 == b.length ? l ? "" : a._allowedJmolSize[2] : b.indexOf("%") == b.length - 1 ? b : 1 >= (e = parseFloat(e)) && 0 < e ? 100 * e + "%" : (isNaN(e = Math.floor(e)) ? a._allowedJmolSize[2] : e < a._allowedJmolSize[0] ? a._allowedJmolSize[0] : e > a._allowedJmolSize[1] ? a._allowedJmolSize[1] :
                e) + (l ? l : "")
        }
        var l;
    ...
    h._searchDatabase = function(e, h, l) {
        if (this._2dapplet && this._2dapplet._isEmbedded && !a.$(this, "appletdiv:visible")[0]) return this._2dapplet._searchDatabase(e, h, l);
        this._showInfo(!1);
    ...
    h.__loadModel = function(e, h, l) {
        null != e && (null != this._viewSet && (h || (h = ""), h += ";if ({*}.molecule.max > 1 || {*}.modelindex.max > 0){ delete molecule > 1 or modelindex > 0;x = getProperty('extractModel',{*});load inline @x};"), !h && this._noscript ? this._applet.loadInlineString(e, "", !1) : this._loadMolData(e, h, !1), null != this._viewSet && a.View.updateView(this, {
            chemID: l,
            data: e
        }))
    };
    ...
    h._isDeferred = function() {
        return !this._canvas && this._cover &&
            this._isCovered && this._deferApplet
    };
    h._checkDeferred = function(a) {
        return this._isDeferred() ? (this._coverScript = a, this._cover(!1), !0) : !1
    };
    h._cover = function(e) {
        e || !this._deferApplet ? this._displayCoverImage(e) : (e = this._coverScript ? this._coverScript : "", this._coverScript = "", this._deferUncover && (e += ";refresh;javascript " + this._id + "._displayCoverImage(false)"), this._script(e, !0), this._deferUncover && "activate 3D model" == this._coverTitle && (a._getElement(this, "coverimage").title = "3D model is loading..."), this._isJava ||
            this._newCanvas(!1), this._defaultModel && a._search(this, this._defaultModel), this._showInfo(!1), this._deferUncover || this._displayCoverImage(!1), this._isJava && a.$html(a.$(this, "appletdiv"), this._javaCode), this._init && this._init())
    };
...
(function(a) {
    ...
    m._radio = function(a, h, e, k, q, s, u, b) {
        ...
        m._actions[u] =
            m._addScript(c, h);
        h = '<span id="span_' + u + '"' + (b ? ' title="' + b + '"' : "") + "><input name='" + s + "' id='" + u + "' type='radio' onclick='Jmol.controls._click(this);return true;' onmouseover='Jmol.controls._mouseOver(this);return true;' onmouseout='Jmol.controls._mouseOut()' " + (k ? "checked='true' " : "") + m._radioCssText + " />";
        0 <= e.toLowerCase().indexOf("<td>") && (h += a, a = "");
        return h + ('<label for="' + u + '">' + e + "</label>" + a + q)
    };
    m._scriptExecute = function(l, h) {
        var e = a._applets[h[0]],
            k = h[1];
        if ("object" == typeof k) k[0](l, k,
            e);
        else "function" == typeof k ? k(e) : a.script(e, k)
    };
    ...
    m._commandKeyPress = function(l, h, e) {
        ...
        switch (l) {
            case 13:
                return h = k.value, m._scriptExecute(k, [e, h]), k._cmds || (k._cmds = [], k._cmddir = 0, k._cmdpt = -1, k._cmdadd = 0), h && 0 == k._cmdadd ? (++k._cmdpt, k._cmds.splice(k._cmdpt, 0, h), k._cmdadd = 0, k._cmddir = 0) : k._cmdadd = 0, k.value = "", !1;
            case 27:
    ...
    m._cbNotifyMaster = function(a) {
        var h = !0,
            ...
            k = !1,
            q, s;
        for (s in a.chkGroup) q = a.chkGroup[s], q.checked ? e = !1 : h = !1, q.indeterminate && (k = !0);
        q = a.chkMaster;
        h ? q.checked = !0 : e ? q.checked = !1 : k = !0;
        q.indeterminate = k;
        (a = m._checkboxItems[q.id]) &&
        (q = a.chkMaster) && m._cbNotifyMaster(m._checkboxMasters[q.id])
    };
    m._cbNotifyGroup = function(a, h) {
        for (var e in a.chkGroup) {
            var k = a.chkGroup[e];
            k.checked != h && (k.checked = h, m._cbClick(k));
            m._checkboxMasters[k.id] && m._cbNotifyGroup(m._checkboxMasters[k.id], h)
        }
    };
    m._cbSetCheckboxGroup = function(a, h, e) {
        var k = a;
        "number" == typeof k && (k = "jmolCheckbox" + k);
        (a = document.getElementById(k)) || alert("jmolSetCheckboxGroup: master checkbox not found: " + k);
        var q = m._checkboxMasters[k] = {};
        q.chkMaster = a;
        q.chkGroup = {};
        "string" ==
        typeof h ? (h = e, k = 1) : k = 0;
        for (a = k; a < h.length; a++) k = h[a], "number" == typeof k && (k = "jmolCheckbox" + k), (e = document.getElementById(k)) || alert("jmolSetCheckboxGroup: group checkbox not found: " + k), q.chkGroup[k] = e, m._checkboxItems[k] = q
    };
    ...
    m._getButton = function(l, h, e, k, q) {
        l = m._getIdForControl(l, h);
        if (null == l) return "";
        void 0 != k && null != k || (k = "jmolButton" + m._buttonCount);
        void 0 != e && null != e || (e = h.substring(0, 32));
        ++m._buttonCount;
        m._actions[k] = m._addScript(l, h);
        h = '<span id="span_' + k + '"' + (q ? ' title="' + q + '"' : "") + "><input type='button' name='" + k + "' id='" + k + "' value='" + e + "' onclick='Jmol.controls._click(this)' onmouseover='Jmol.controls._mouseOver(this);return true' onmouseout='Jmol.controls._mouseOut()' " +
            m._buttonCssText + " /></span>";
        a._debugAlert && alert(h);
        return a._documentWrite(h)
    };
    m._getCheckbox = function(l, h, e, k, q, s, u) {
        ...
        else if (void 0 == k || null == k) alert("jmolCheckbox requires a label");
        else return m._actions[s] = [m._addScript(b, h), m._addScript(b, e)], l = "</span>",
            q = '<span id="span_' + s + '"' + (u ? ' title="' + u + '"' : "") + "><input type='checkbox' name='" + s + "' id='" + s + "' onclick='Jmol.controls._cbClick(this)' onmouseover='Jmol.controls._cbOver(this);return true' onmouseout='Jmol.controls._mouseOut()' " + (q ? "checked='true' " : "") + m._checkboxCssText + " />", 0 <= k.toLowerCase().indexOf("<td>") && (q += l, l = ""), q += '<label for="' + s + '">' + k + "</label>" + l, a._debugAlert && alert(q), a._documentWrite(q)
    };
    m._getCommandInput = function(l, h, e, k, q, s) {
        l = m._getIdForControl(l, "x");
        if (null == l) return "";
        void 0 != k && null != k || (k = "jmolCmd" + m._cmdCount);
        void 0 != h && null != h || (h = "Execute");
        void 0 != e && !isNaN(e) || (e = 60);
        void 0 != s || (s = "help");
        ++m._cmdCount;
        h = '<span id="span_' + k + '"' + (q ? ' title="' + q + '"' : "") + "><input name='" + k + "' id='" + k + "' size='" + e + "' onkeydown='return Jmol.controls._commandKeyPress(event,\"" + k + '","' + l + "\")' value='" + s + "'/><input  type='button' name='" + k + "Btn' id='" + k + "Btn' value = '" + h + "' onclick='Jmol.controls._commandKeyPress(13,\"" + k + '","' + l + "\")' /></span>";
        a._debugAlert && alert(h);
        return a._documentWrite(h)
    };
    m._getLink = function(l, h, e, k, q) {
        l = m._getIdForControl(l, h);
        if (null == l) return "";
        void 0 != k && null != k || (k = "jmolLink" + m._linkCount);
        void 0 != e && null != e || (e = h.substring(0, 32));
        ++m._linkCount;
        h = m._addScript(l, h);
        e = '<span id="span_' + k + '"' + (q ? ' title="' + q + '"' : "") + "><a name='" + k + "' id='" + k + "' href='javascript:Jmol.controls._click(null," + h + ");' onmouseover='Jmol.controls._mouseOver(null," + h + ");return true;' onmouseout='Jmol.controls._mouseOut()' " + m._linkCssText + ">" + e + "</a></span>";
        a._debugAlert && alert(e);
        return a._documentWrite(e)
    };
    m._getMenu = function(l, h, e, k, q) {
        var s = m._getIdForControl(l, null);
        void 0 != k && null != k || (k = "jmolMenu" + m._menuCount);
        ++m._menuCount;
        s = typeof h;
        if (null != s && "object" == s && h.length) {
            var u = h.length;
            "number" != typeof e || 1 == e ? e = null : 0 > e && (e = u);
            e = '<span id="span_' + k + '"' + (q ? ' title="' + q + '"' : "") + "><select name='" + k + "' id='" + k + "' onChange='Jmol.controls._menuSelected(this)'" + (e ? " size='" + e + "' " : "") + m._menuCssText + ">";
            for (k = 0; k < u; ++k) {
                var b = h[k],
                    s = typeof b,
                    c = null,
                    d = q = null;
                "object" == s && null != b ? (c = b[0], q = b[1], d = b[2]) :
                    c = q = b;
                s = m._getIdForControl(l, c);
                if (null == s) return "";
                null == q && (q = c);
                "#optgroup" == c ? e += "<optgroup label='" + q + "'>" : "#optgroupEnd" == c ? e += "</optgroup>" : (s = m._addScript(s, c), e += "<option value='" + s + (d ? "' selected='true'>" : "'>") + q + "</option>")
            }
            e += "</select></span>";
            a._debugAlert && alert(e);
            return a._documentWrite(e)
    ...
    m._getRadioGroup = function(l, h, e, k, q, s) {
        ...
        if ("object" != u || null == u || !h.length) alert("invalid arrayOfRadioButtons");
            ...
            var b = h.length;
            ++m._radioGroupCount;
            k || (k = "jmolRadioGroup" + (m._radioGroupCount - 1));
            for (var c = "<span id='" + (q ? q : k) + "'>", d = 0; d < b; ++d) {
                d == b - 1 && (e = "");
                var t = h[d],
                    u = typeof t,
                    F = null,
                    c = "object" == u ? c + (F = m._radio(l, t[0], t[1], t[2], e, k, 3 < t.length ? t[3] : (q ? q : k) + "_" + d, 4 < t.length ? t[4] : 0,
                        s)) : c + (F = m._radio(l, t, null, null, e, k, (q ? q : k) + "_" + d, s));
                if (null == F) return ""
            }
            c += "</span>";
            a._debugAlert && alert(c);
...
(function(a) {
    ...
    a.getJMEApplet = function(l, h, e, k) {
        return a._JMEApplet._get(l, h, e, k)
    };
    ...
    a.jmolButton = function(l, h, e, k, m) {
        return a.controls._getButton(l, h, e, k, m)
    };
    a.jmolCheckbox = function(l, h, e, k, m, s, u) {
        return a.controls._getCheckbox(l, h, e, k, m, s, u)
    };
    a.jmolCommandInput = function(l, h, e, k, m, s) {
        return a.controls._getCommandInput(l, h, e, k, m, s)
    };
    a.jmolHtml = function(l) {
        return a._documentWrite(l)
    };
    a.jmolLink = function(l, h, e, k, m) {
        return a.controls._getLink(l, h, e, k, m)
    };
    a.jmolMenu = function(l, h, e, k, m) {
        return a.controls._getMenu(l, h, e, k, m)
    };
    a.jmolRadio = function(l, h, e, k, m, s, u, b) {
        return a.controls._getRadio(l,
            h, e, k, m, s, u, b)
    };
    a.jmolRadioGroup = function(l, h, e, k, m, s) {
        return a.controls._getRadioGroup(l, h, e, k, m, s)
    };
...
LoadClazz = function() {
    ...
    window["j2s.clazzloaded"] || (window["j2s.clazzloaded"] = !0, window["j2s.object.native"] = !0, Clazz = {
        ...
        k.finalize = function() {};
        k.notify = function() {};
        k.notifyAll = function() {};
        k.wait = function() {};
        k.to$tring = Object.prototype.toString;
        k.toString = function() {
        ...
        a.inheritClass = function(b, c, d) {
            for (var e in c) "b$" != e && ("prototype" != e && "superClazz" != e && "__CLASS_NAME__" != e && "implementz" != e && !q(c, e)) && (b[e] = c[e]);
        ...
        var u = function(a, b) {
            for (var c in b)
                if ("b$" != c && "prototype" != c && "superClazz" != c && "__CLASS_NAME__" != c && "implementz" !=
                    c && ("function" != typeof b[c] || !q(b, c))) a[c] = a.prototype[c] = b[c]
        };
        ...
        a.getInheritedLevel = function(b, c) {
            ...
            for (e = b; e !== c && 10 > d;) {
                if (e.implementz)
                    for (var h = e.implementz, k = 0; k < h.length; k++) {
                        var l = a.equalsOrExtendsLevel(h[k], c);
                        if (0 <= l) return d + l + 1
                    }
                e = e.superClazz;
        ...
        a.superCall = function(b, c, d, e) {
            ...
            if (l)
                ...
                for (k = m.length; 0 <= --k;)
                    if (c === m[k]) {
                        h = 0 < k ? m[--k].prototype[d] : m[0].prototype[d]["\\unknown"];
                        break
                    } else if (0 < a.getInheritedLevel(c, m[k])) {
                    h = m[k].prototype[d];
                    break
                }
            }
            if (h) return 0 == k && "construct" == d && (c = l.stacks) && (!c[0].superClazz && c[0].con$truct) && c[0].con$truct.apply(b, []), h.apply(b,
                e || []);
        ...
        try {
            ...
            if (k = function(a, b, c) {
                ...
                var k = d.message.indexOf(":"),
                    t = d.message.indexOf(":", k + 2),
                    F = d.message.substr(k + 1, t - k - 20);
                c = function(a) {
                    return -1 != a.message.indexOf(F)
                }
            } else if (-1 != navigator.userAgent.toLowerCase().indexOf("webkit")) {
                var C = k(d.message, "hello");
                c = function(a) {
                    return C.test(a.message)
                }
            } else C = k(d.message, "$$o$$"), c = function(a) {
                return C.test(a.message)
            }
        }
        ...
        a.getStackTrace = function(a) {
            ...
            for (var e = 0; e < a && (c = c.caller); e++) {
                ...
                if (d)
                    for (var h = c.arguments, k = 0; k < h.length; k++) {
                        var l = "" + h[k];
                        60 < l.length && (l = l.substring(0, 60) + "...");
                        b += " args[" + k + "]=" + l.replace(/\s+/g, " ") + "\n"
                    }
            }
            return b
        };
        ...
        a.defineMethod = function(j, c, d, h) {
            d.exName = c;
            h = Ea(h);
            var k = j.prototype,
                l = k[c];
            a._Loader._checkLoad && K(j, c, h);
            if (!l || l.claxxOwner === j && l.funParams == h) return d.funParams = h, d.claxxOwner = j, d.exClazz = j, k[c] = d;
            var m = null,
                q = l.stacks;
            q || (q = [], m = l, l.claxxOwner && (q[0] =
                m.claxxOwner));
            if (!l.stacks || l.claxxReference !== j) {
                ++P;
                l = function() {
                    var j;
                    a: {
                        var c = arguments.callee.claxxReference,
                            d = arguments.callee.methodName;j = arguments;fx = this[d];
                        var h = a.getParamsType(j);
                        if (!fx) try {
                            System.out.println(a.getStackTrace(5))
                        } catch (n) {}
                        if (e) {
                            var k = c.__CLASS_NAME__ + " " + d + " ";
                            0 > U.indexOf(k) && (U += k + "\n");
                            e[k] || (e[k] = 0);
                            e[k]++
                        }
                        if (fx.lastParams == h.typeString && fx.lastClaxxRef === c) {
                            if (h.hasCastedNull) {
                                c = [];
                                for (d = 0; d < j.length; d++) c[d] = j[d] instanceof a.CastedNull ? null : j[d]
                            } else c = j;
                            j = fx.lastMethod ?
                                fx.lastMethod.apply(this, c) : null
                        } else {
                            fx.lastParams = h.typeString;
                            fx.lastClaxxRef = c;
                            k = fx.stacks;
                            k || (k = c.prototype[d].stacks);
                            for (var r = !1, l = k.length; 0 <= --l;)
                                if (r || k[l] === c) {
                                    var m = k[l].prototype[d],
                                        z = h,
                                        r = j,
                                        q = fx,
                                        s = [],
                                        ...
                                        G = void 0;
                                    for (G in m)
                                        if (92 == G.charCodeAt(0)) {
                                            var u = G.substring(1).split("\\");
                                            u.length == z.length && s.push(u);
                                            t = !1
                                        } else if (t && "funParams" == G && m.funParams) {
                                        G = m.funParams;
                                        u = G.substring(1).split("\\");
                                    ...
                                    if (!(G = 0 == s.length)) {
                                        G = void 0;
                                        for (var G = [],
                                                Q = s.length, u = 0; u < Q; u++) {
                                            for (var x = [], za = !0, D = s[u].length, C = 0; C < D; C++)
                                                if (x[C] = a.getInheritedLevel(z[C], s[u][C]), 0 > x[C]) {
                                                    za = !1;
                                                    break
                                                }
                                            za && (x[z.length] = u, G.push(x))
                                        }
                                        if (0 == G.length) G = null;
                                        else {
                                            Q = G[0];
                            ...
                            "construct" != d && fb(this, c, d, h.typeString);
                            j = void 0
                        }
                    }
                    return j
                ...
                l = k[c] = l;
                c = [];
                for (k = 0; k < q.length; k++) c[k] = q[k];
                l.stacks = c
            }
            q = l.stacks;
            0 > rb(q, j) && q.push(j);
            m && (m.claxxOwner === j ? (l[m.funParams] = m, m.claxxOwner = null, m.funParams = null) : m.claxxOwner || (l["\\unknown"] = m));
            d.exClazz = j;
            l[h] = d;
            return l
        };
        ...
        var K = function(b, c, d) {
            var e = b.prototype[c];
            if (e && (e.claxxOwner || e.claxxReference) === b) key = b.__CLASS_NAME__ + "." + c + d, (b = duplicatedMethods[key]) ?
                (c = "Warning! Duplicate method found for " + key, System.out.println(c), a.alert(c), duplicatedMethods[key] = b + 1) : duplicatedMethods[key] = 1
        };
        ...
        var rb = function(a, b) {
            ...
            Ea = function(a) {
                return a ?
                    a.replace(/~([NABSO])/g, function(a, b) {
                        switch (b) {
                            case "N":
                                return "n";
                            case "B":
        ...
        a.defineType = function(b, c, d, e) {
            ...
            h = b.lastIndexOf(".");
            if (-1 != h) {
                var k = b.substring(0, h),
                    k = a.declarePackage(k),
                    h = b.substring(h + 1);
                if (k[h]) return k[h];
                k[h] = c
            } else {
        ...
        var Fa = !1; - 1 != navigator.userAgent.indexOf("Safari") && (k = navigator.userAgent, t = k.indexOf("Version/"), -1 != t && (k = k.substring(t + 8), Fa = 4 <= parseFloat(k)));
        a.instantialize = function(a, b) {
            if (!b || !(1 == b.length && b[0] && b[0] instanceof s)) {
                ...
                if (Fa) {
                    ...
                    b =
                        c
                }(c = a.construct) ? a.con$truct ? a.getClass().superClazz ? c.claxxOwner && c.claxxOwner === a.getClass() || c.stacks && c.stacks[c.stacks.length - 1] == a.getClass() ? c.apply(a, b) : (c.claxxOwner && !c.claxxOwner.superClazz && c.claxxOwner.con$truct ? c.claxxOwner.con$truct.apply(a, []) : c.stacks && (1 == c.stacks.length && !c.stacks[0].superClazz) && c.stacks[0].con$truct.apply(a, []), c.apply(a, b), a.con$truct.apply(a, [])) : (a.con$truct.apply(a, []), c.apply(a, b)) : c.apply(a, b): a.con$truct && a.con$truct.apply(a, [])
            }
        };
            ...
            getResourceAsStream: function(b) {
                ...
                if (0 == b.indexOf("/"))
                    ...
                    if (this.base) c = this.base;
                        ...
                        for (var c = c.substring(0,
                                e + 1), e = d.split(/\./), h = 1; h < e.length; h++) {
                            for (var k = "/", l = 0; l < h; l++) k += e[l] + "/";
                            if (k.length > c.length) break;
                            if (c.indexOf(k) == c.length - k.length) {
                                c = c.substring(0, c.length - k.length + 1);
                                break
                            }
        ...
        a.decorateAsClass = function(b, c, d, e, h, k) {
            var l = null;
            ...
            db(b, c, d);
            k ? a.inheritClass(b, e, k) : e && a.inheritClass(b, e);
            h && a.implementOf(b,
                h);
            return b
        };
        ...
        a.decorateAsType = function(b, c, d, e, h, k) {
            a.extendJO(b, c);
            b.equals = a._innerFunctions.equals;
            b.getName = a._innerFunctions.getName;
            if (k)
                for (c = 0; c < a.innerFunctionNames.length; c++) k = a.innerFunctionNames[c], b[k] = a._innerFunctions[k];
            h ? a.inheritClass(b, d, h) : d && a.inheritClass(b, d);
            e && a.implementOf(b, e);
            return b
        };
        Number.prototype._numberToString =
        ...
        a.innerTypeInstance = function(b, c, d) {
            ...
            for (h = Array(k); 0 <= --k;) h[k] = arguments[k + 3];
            a.instantialize(e, h);
            return e
        };
        a.cloneFinals = function() {
            for (var a = {}, b = arguments.length / 2; 0 <= --b;) a[arguments[b +
                b]] = arguments[b + b + 1];
            return a
        };
        ...
        var gb = function(a, b) {
                ...
                if ("object" == typeof a) var c = a;
                    ...
                    for (var c = Array(a), d = 0; d < a; d++) c[d] = 0;
        ...
        !0 == (a.haveInt32 = !!(self.Int32Array && self.Int32Array != Array)) ? Int32Array.prototype.sort || (Int32Array.prototype.sort = Array.prototype.sort) : (Int32Array = function(a) {
            return gb(a, 32)
        }, Int32Array.prototype.sort = Array.prototype.sort, Int32Array.prototype.toString = function() {
        ...
        !0 == (a.haveFloat64 = !!(self.Float64Array && self.Float64Array != Array)) ? Float64Array.prototype.sort || (Float64Array.prototype.sort = Array.prototype.sort) : (Float64Array = function(a) {
            return gb(a, 64)
        }, Float64Array.prototype.sort = Array.prototype.sort, Float64Array.prototype.toString = function() {
        ...
        a.newCharArray = a.newBooleanArray =
            a.newArray;
        ...
        a.isAF = function(a) {
            return a &&
                "object" == typeof a && 8 == a.BYTES_PER_ELEMENT
        };
        a.isAS = function(a) {
        ...
        a.isAFloat = function(b) {
            return b && "object" == typeof b && b.constructor == Array && a.instanceOf(b[0],
                Float)
        };
        a.isAP = function(b) {
        ...
        a.prepareFields = function(a, b) {
            ...
            d = a.con$truct = function() {
                ...
                if (a)
                    for (var b = 0; b < a.length; b++) a[b].apply(this, [])
            };
        ...
        k = a.innerFunctionNames;
        for (t = 0; t < k.length; t++) a._O[k[t]] = a._innerFunctions[k[t]], Array[k[t]] = a._innerFunctions[k[t]];
        ...
        (function(a, b) {
            ...
            b.updateNodeForFunctionDecoration = function(a) {
                (a = I(a)) && a.status == x.STATUS_KNOWN && window.setTimeout(function(a) {
                        return function() {
                            updateNode(a)
                        }
                    }(a),
            ...
            var d = new x,
                ...
                g = -1 != l.indexOf("gecko");
            if (q && (k = 1, q = l.indexOf("opera/"), -1 != q)) {
                var t = 9;
                try {
            ...
            var C;
            self.Clazz && a.isClassDefined ? isClassDefined = a.isClassDefined : (C = {}, isClassDefined = function(a) {
                return !0 == C[a]
            });
            var D = function(a) {
            ...
            b.loadPackageClasspath =
                function(a, c, d, e, g, h) {
                    ...
                    h || (h = 0);
                    var j = d && E["@" + a];
                    if (0 == g && (d && !E["@java"] && 0 != a.indexOf("java") && null != window["java.registered"] && !E["@java"]) && (b.loadPackage("java", e ? function() {
                            b.loadPackageClasspath(a, c, d, e, 1)
                        } : null), e)) return;
                    if (a instanceof Array)
                        if (D(a), e) h < a.length ? b.loadPackageClasspath(a[h], c, d, function() {
                        ...
                        switch (a) {
                            ...
                            case "java":
                                c &&
                                    (g = "@net.sf.j2s.ajax", E[g] || (E[g] = c), g = "@net.sf.j2s", E[g] || (E[g] = c));
                                break;
                            case "swt":
                                a = "org.eclipse.swt";
                                break;
            ...
            b.loadClass = function(c, g, h, k, l) {
                l || (l = 0);
                null == k && (k = !1);
                if ("boolean" == typeof g) return a.evalType(c);
                null != window["java.registered"] && !E["@java"] && b.loadPackage("java");
                b.keepOnLoading = !0;
                if (!h && (K && c.lastIndexOf(".package") != c.length - 8 || 0 != c.indexOf("java.") && !isClassDefined(ca))) La.push([c, g]), System.out.println("loadclass-queuing" + c + ca + " " + isClassDefined(ca));
                else if ((l = isClassDefined(c)) || O["@" + c]) {
                    if (l && g && (h = I(c), !h || h.status >= x.STATUS_LOAD_COMPLETE)) k ? window.setTimeout(g, 25) : g()
                } else {
                    var m =
                        b.getClasspathFor(c);
                    l = e[m];
                    ...
                    if (l) {
                        ...
                        l = a.unloadedClasses[c] && I(c) || new x;
                        l.name = c;
                        l.path = m;
                        ...
                        c = !1;
                        for (k = F.length; 0 <= --k;)
                            if (F[k].status != x.STATUS_LOAD_COMPLETE) {
                                c = !0;
                                break
                            }
                        if (l.isPackage) {
                            for (k = F.length; 0 <= --k && !F[k].isPackage;) F[k +
                                1] = F[k];
                            F[++k] = l
                        } else c && F.push(l);
                        if (!c) {
            ...
            b.jarClasspath = function(a, b) {
                ...
                for (var c = b.length; 0 <= --c;) E["#" + b[c]] = a;
                E["$" + a] = b
            };
            ...
            b.getClasspathFor = function(c, d, e) {
                ...
                if (!g || d || e) {
                    var h, k;
                    if (g) {
                        if (c = c.replace(/\./g, "/"), 0 <= (k = g.lastIndexOf(c)) || 0 <= (k = c.lastIndexOf("/")) && 0 <= (k = g.lastIndexOf(c.substring(0, k)))) h = g.substring(0, k)
                    } else {
                        for (k = c.length + 2; 0 <= (k = c.lastIndexOf(".", k - 2)) && !(h = E["@" + c.substring(0, k)]););
                        d || (c = c.replace(/\./g, "/"))
                    }
                    ...
                    g = (h.lastIndexOf("/") == h.length - 1 ? h : h + "/") + (d ? "" : c.lastIndexOf("/*") == c.length - 2 ? c.substring(0, k + 1) : c + (!e ? ".js" : "." != e.charAt(0) ? "." + e : e))
                }
                return g
            };
            b.ignore = function() {
            ...
            var L = {},
                ...
                Z = function(a) {
                    window["j2s.script.debugging"] || window.setTimeout(aa(a), 1)
                };
            ...
            var V = function(c,
                    ...
                    if (M && !ra) {
                        ...
                        k ? setTimeout(c.success, 0) : m.$ajax(c)
                },
                ta = function(c, d, e) {
                    a.getStackTrace();
                    return function() {
                    ...
                    if (l) {
                        ...
                        if (r)
                            for (var q = 0; q < r.length; q++) {
                                var t =
                                    r[q];
                                if (t != l.name && (m = I(t))) m.status < x.STATUS_CONTENT_LOADED && (m.status = x.STATUS_CONTENT_LOADED, updateNode(m));
                                else {
                                    m = new x;
                                    m.name = t;
                                    var u = E["#" + t];
                        ...
                        if (b.keepOnLoading) {
                            ...
                            if (m = Ka(x.STATUS_KNOWN))
                                for (S(m); h < k && (m = Ka(x.STATUS_KNOWN));) S(m);
                            else if (0 != F.length) m = F.shift(), !e[m.path] || 0 != F.length || !ga || m.musts.length || m.optionals.length ? (Ja(d, m, !0), V(m, m.path,
                                m.requiredBy, !1)) : ga && (ga = !1);
                            else if (m = H(x.STATUS_KNOWN))
                                for (S(m); h < k && (m = H(x.STATUS_KNOWN));) S(m);
                            else q = !1;
                            if (!(q || 0 < h)) {
                                l = [Ka, H];
                                r = null;
                                ...
                                if (g) g();
                                    ...
                                    for (t in b._classPending) {
                                        if (m = I(t), System.out.println("class left pending " + t + " " + m), m) {
                                            updateNode(m);
                                            break
                                        }
                                    } else b._checkLoad && (System.out.println("I think I'm done: SAEM call count: " + P), a.showDuplicates(!0));
                                b.onGlobalLoaded()
                            }
                        }
                    }
                ...
                ma = function(a, c) {
                    ...
                    for (h = 0; 2 > h; h++) {
                        k = j[h];
                        for (g = k.length; 0 <= --g;)
                            if (k[g].status == x.STATUS_DECLARED && ma(k[g], c)) return !0
                    }
                    d.length = e;
                    return !1
                };
                        ...
                        if (a.status < x.STATUS_DECLARED) {
                            ...
                            if (a.declaration && a.declaration.clazzList) {
                                ...
                                for (l = k.length; j < l; j++)
                                    if ((m = I(k[j])) && m.status != x.STATUS_DECLARED && m !== a) m.status = x.STATUS_DECLARED, C && (C[m.name] = !0), b.onScriptInitialized(m.path)
                            }
                        }
                        c = x.STATUS_DECLARED;
            ...
            var Ta = function(a, b) {
                ...
                qa = function(a,
                    ...
                    for (var c = a.musts.length; 0 <= --c;) {
                        var d = a.musts[c];
                        if (Aa(d, b) || (d = qa(d, b))) return d
                    }
                    return Aa(a, b) ? a : null
                },
                Ca = function(a, b) {
                    var c;
                    return (c = na(a.musts, b)) || (c = na(a.optionals, b)) || Aa(c = a, b) ? c : null
                },
                na = function(a, b) {
                    if (a)
                ...
                R = function(a, c, e, g) {
                    if (c instanceof Array) {
                        ...
                        h = L["#" + c];
                        h || (h = (h = I(c)) ? h : new x, h.name = c, h.path = E["#" + c] || "unknown", X(h.path, c, h), h.status = x.STATUS_KNOWN, Ja(d, h, !1));
                        ia(h, a, !0);
            ...
            var X = function(a, b, c) {
                ...
                ca = b.runtimeKeyClass = "java.lang.String",
                La = [];
            ...
            b.loadZJar = function(a, c) {
                var d = null,
                    e = c instanceof
                Array;
                e ? c = c[c.length - 1] : d = c == ca ? ba : null;
            ...
            var Fa = {},
                ...
                Ja = function(a, b, c) {
                    ...
                    e || (c.push(b), ga && (0 != b.name.indexOf("java") && 0 != b.name.indexOf("net.sf.j2s.ajax")) && (fa && (ga = !1), fa = !0));
                    a: {
                        if (a.name && a != d && a != b)
        ...
        var Z = a._LoaderProgressMonitor,
            L = null,
            fa = 0,
            O = null,
            wa = 0;
        Z.DEFAULT_OPACITY = m && m._j2sLoadMonitorOpacity ? m._j2sLoadMonitorOpacity : 55;
        Z.hideMonitor = function() {
            O.style.display = "none"
        };
        Z.showStatus = function(a, b) {
            if (!O) {
            ...
            jb(O);
            if (null == a) b ? ma() : Z.hideMonitor();
            else {
                O.appendChild(document.createTextNode("" + a));
                "none" == O.style.display && (O.style.display = "");
                kb(Z.DEFAULT_OPACITY);
                var d, c = navigator.userAgent;
                d = document.body;
                var e = d.parentNode,
                    h = e.clientHeight;
                d = d.scrollTop + d.offsetTop;
                var k = e.scrollTop + e.offsetTop,
                    c = 0 > c.indexOf("Opera") && document.all ? 0 == h ? d : k : 0 > c.indexOf("Gecko") ? h == e.offsetHeight && h == e.scrollHeight ? d : k : d;
        ...
        var jb = function(a) {
            ...
            kb = function(a) {
                L && a == Z.DEFAULT_OPACITY && (window.clearTimeout(L), L = null);
                fa = a;
            ...
            ya = !1,
            ma = function() {
                "none" != O.style.display && (fa == Z.DEFAULT_OPACITY ? (L = window.setTimeout(function() {
                    ma()
                }, 750), fa -= 5) : 0 <= fa - 10 ? (kb(fa - 10), L = window.setTimeout(function() {
                    ma()
            ...
            aa = System;
        D.maxTotalLines = 1E4;
        D.setMaxTotalLines = function(a) {
            D.maxTotalLines = 0 < a ? a : 999999
        };
        D.maxLatency = 40;
        D.setMaxLatency = function(a) {
            D.maxLatency = 0 < a ? a : 40
        };
        D.pinning = !1;
        D.enablePinning = function(a) {
            D.pinning = a
        };
        D.linesCount = 0;
        D.metLineBreak = !1;
        D.createConsoleWindow =
            function() {
        ...
        var V = String.fromCharCode(160),
            V = V + (V + V + V);
        D.consoleOutput = function(a, b) {
            var c = window["j2s.lib"];
            (c = c && c.console) && "string" == typeof c && (c = document.getElementById(c));
            if (!c) return !1;
            if (D.linesCount > D.maxTotalLines) {
                for (var d = 0; d < D.linesCount - D.maxTotalLines; d++) c && 0 < c.childNodes.length && c.removeChild(c.childNodes[0]);
                D.linesCount = D.maxTotalLines
            }
            var e = !1;
            ...
            var h = null;
            a = a.replace(/\t/g, V);
            for (var h = a.split(/\r\n|\r|\n/g), d = 0, k = h.length - 1; d <= k; d++) {
                var l = null;
                if (D.metLineBreak || 0 == D.linesCount || 1 > c.childNodes.length) l = document.createElement("DIV"), c.appendChild(l), l.style.whiteSpace = "nowrap", D.linesCount++;
                else try {
                    l = c.childNodes[c.childNodes.length -
                        1]
                } catch (m) {
                    l = document.createElement("DIV"), c.appendChild(l), l.style.whiteSpace = "nowrap", D.linesCount++
                }
                ...
                D.pinning || (c.scrollTop += 100);
                D.metLineBreak = d != k || e
            }
            d = c.parentNode.className;
            !D.pinning && (d && -1 != d.indexOf("composite")) && (c.parentNode.scrollTop = c.parentNode.scrollHeight);
            D.lastOutputTime = (new Date).getTime()
        };
        D.clear = function() {
            try {
                D.metLineBreak = !0;
        ...
        aa.out.print = function(a) {
            D.consoleOutput(a)
        };
        aa.out.println = function(a) {
            D.consoleOutput("undefined" == typeof a ? "\r\n" : null == a ? "null\r\n" : a + "\r\n")
        };
        aa.out.write = function(a, b, c) {
            aa.out.print(String.instantialize(a).substring(b, b + c))
        };
        aa.err.__CLASS_NAME__ = "java.io.PrintStream";
        aa.err.print =
            function(a) {
                D.consoleOutput(a, "red")
            };
        aa.err.println = function(a) {
            D.consoleOutput("undefined" == typeof a ? "\r\n" : null == a ? "null\r\n" : a + "\r\n", "red")
        };
        aa.err.write = function(a, b, c) {
            aa.err.print(String.instantialize(a).substring(b, b + c))
        }
    }(Clazz, Jmol))
};
